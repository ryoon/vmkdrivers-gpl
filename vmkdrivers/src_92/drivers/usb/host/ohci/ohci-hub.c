...
#define put_unaligned_le32(temp, buf)	\
...
#ifdef	CONFIG_PM
#if defined(__VMKLNX__)
static int ohci_rh_suspend (struct ohci_hcd *ohci, int autostop, unsigned long *flags)
#else
...
#if defined(__VMKLNX__)
static int ohci_rh_resume(struct ohci_hcd *ohci, unsigned long *flags)
#else
...
#if defined(__VMKLNX__)
		rc = ohci_rh_suspend(ohci, 0, &flags);
#else
...
#if defined(__VMKLNX__)
		ohci_rh_resume(ohci, &flags);
#else
		ohci_rh_resume(ohci);
#endif
		hcd->state = HC_STATE_QUIESCING;
#if defined(__VMKLNX__)
		ohci_rh_suspend(ohci, 0, &flags);
#else
...
static int ohci_root_hub_state_changes(struct ohci_hcd *ohci, int changed,
                int any_connected, int rhsc_status, unsigned long *flags)
#else
static int ohci_root_hub_state_changes(struct ohci_hcd *ohci, int changed,
...
ohci_hub_status_data (struct usb_hcd *hcd, char *buf)
{
	...
	if (ohci_root_hub_state_changes(ohci, changed,
			any_connected, rhsc_status, &flags))
#else
	if (ohci_root_hub_state_changes(ohci, changed,
