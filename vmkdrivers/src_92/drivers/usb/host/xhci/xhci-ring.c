...
// XXX Avoid log spew in VMKLNX build
...
#if defined(__VMKLNX__)
	xhci_info(xhci, "NEC firmware version %2x.%02x",
...
hw_died:
...
#if defined (__VMKLNX__)
	{
		...
		/*
		 * Acknowledge the interrupt. Because we might be in polling
		 * mode we can't rely on MSI/MSI-X clearing it for us
		 * automatically.
		 */
		if (ER_IRQ_PENDING(irq_pending)) {
			irq_pending |= IMAN_IP;
