 ...
 *
 * VMKLNX note:
 * power. changed to power2-> throughout where needed; see pm.h.
 */
...
extern atomic_t in_usb_passthrough;
#endif
...
#if defined(__VMKLNX__)
 	if (udev->in_use) {
 		dev_info(&udev->dev, "Device is allocated for USB passthrough "
				     "use; not available for VMkernel use\n");
 		return error;
 	}
#endif
...
#if defined(__VMKLNX__)
	VMKAPI_MODULE_CALL_VOID(udriver->module->moduleID,
...
#if defined(__VMKLNX__)
static void update_passthrough(struct usb_interface *iface)
{
...
#if defined(__VMKLNX__)
	VMKAPI_MODULE_CALL_VOID(driver->module->moduleID,
...
#if !defined(__VMKLNX__) || defined(CONFIG_PM_SLEEP)
        /*
         * VMKLNX note: The "hs_companion" support in hcd-pci.c is
         * under CONFIG_PM_SLEEP, so it should not be needed here
         * without CONFIG_PM_SLEEP.  Also, when CONFIG_PM_RUNTIME is
         * defined but CONFIG_PM_SLEEP is not, all events should have
         * PM_EVENT_AUTO set.
         */
...
#if defined(__VMKLNX__)
	if (status)
		dev_err(&udev->dev, "%s: failed with error %d\n",
			__func__, status);
#endif
	return status;
}
...
void usb_autosuspend_device(struct usb_device *udev)
{
	...
	dev_vdbg(&udev->dev, "%s: cnt %d -> %d\n",
			__func__, atomic_read(&udev->dev.power2->usage_count),
			status);
}
...
void usb_autopm_put_interface(struct usb_interface *intf)
{
	...
	dev_vdbg(&intf->dev, "%s: cnt %d -> %d\n",
			__func__, atomic_read(&intf->dev.power2->usage_count),
			status);
}
...
void usb_autopm_put_interface_async(struct usb_interface *intf)
{
	...
	if (!udev->autosuspend_disabled) {
		 ...
		 */
		if (intf->dev.power2->timer_expires == 0 ||
...
int usb_autopm_get_interface_async(struct usb_interface *intf)
{
	...
	s = ACCESS_ONCE(intf->dev.power2->runtime_status);
...
static int autosuspend_check(struct usb_device *udev)
{
	...
	if (udev->actconfig) {
		for (i = 0; i < udev->actconfig->desc.bNumInterfaces; i++) {
			 ...
			 */
			if (intf->dev.power2->disable_depth)
				continue;
			if (atomic_read(&intf->dev.power2->usage_count) > 0)
				return -EBUSY;
