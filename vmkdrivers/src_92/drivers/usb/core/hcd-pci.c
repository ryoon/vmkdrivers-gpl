...
int usb_hcd_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	/* can't count on this being done elsewhere (PR487524) */
...
#if defined(__VMKLNX__)
		VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, retval,
			hcd->driver->pci_suspend, hcd);
#else
...
#if defined(__VMKLNX__)
		VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, retval,
			hcd->driver->pci_resume, hcd, hibernated);
#else
