...
#if defined(__VMKLNX__)
		VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, status,
...
void usb_hcd_poll_rh_status(struct usb_hcd *hcd)
{
	...
	if ((hcd->self.root_hub->state == USB_STATE_ATTACHED) &&
...
#if defined(__VMKLNX__)
	VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, length,
...
#if defined(__VMKLNX__)
		VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, value,
		...
		struct usb_host_interface *new_alt)
{
	...
	if (new_config) {
		...
		for (i = 1; i < 16; ++i) {
			...
			if (ep) {
#if defined (__VMKLNX__)
				VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, ret,
					hcd->driver->drop_endpoint, hcd, udev, ep);
#else
...
#if defined (__VMKLNX__)
				VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, ret,
					hcd->driver->add_endpoint, hcd, udev, &alt->endpoint[j]);
#else
...
#if defined (__VMKLNX__)
			VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, ret,
				hcd->driver->drop_endpoint, hcd, udev,
				&cur_alt->endpoint[i]);
#else
...
#if defined (__VMKLNX__)
			VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, ret,
				hcd->driver->add_endpoint, hcd, udev,
				&new_alt->endpoint[i]);
#else
...
#if defined(__VMKLNX__)
		VMKAPI_MODULE_CALL(hcd->driver->module->moduleID, status,
			hcd->driver->start_port_reset, hcd, port_num);
#else
...
#if defined(__VMKLNX__)
/* called from hid_probe() in a context of khubd thread */
void vmklnx_hcd_register_kbd_handler(struct usb_interface *intf)
{
	vmklnx_usb_interrupt_info int_info;
	struct usb_hcd *hcd = bus_to_hcd(interface_to_usbdev(intf)->bus);

	if (hcd->kbd_handle)
		return;
	
	int_info.handler = (vmklnx_usb_kbd_int_handler *)usb_hcd_irq;
	int_info.irq = hcd->irq;
	int_info.context = (void *)hcd;
	int_info.regs = NULL; //unused
			
	hcd->kbd_handle = vmklnx_register_usb_kbd_int_handler(&int_info);
	dev_info(hcd->self.controller, "register keyboard interrupt handler %s\n",
		hcd->kbd_handle ? "succeeded" : "failed");
}
EXPORT_SYMBOL_GPL (vmklnx_hcd_register_kbd_handler);
#endif
