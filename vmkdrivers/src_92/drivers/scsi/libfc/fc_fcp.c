...
static void fc_fcp_recv_data(struct fc_fcp_pkt *fsp, struct fc_frame *fp)
{
	 ...
	 */
	if (fsp->xfer_ddp != FC_XID_UNKNOWN) {
		FC_FCP_DBG(fsp, "XFER :x%x DDP I/O in fc_fcp_recv_data set ERROR \n",
			fsp->xfer_ddp);
		fc_fcp_ddp_done(fsp);
		host_bcode = FC_ERROR;
		goto err;
...
static void fc_fcp_abts_resp(struct fc_fcp_pkt *fsp, struct fc_frame *fp)
{
	...
	case FC_RCTL_BA_RJT:
		...
		if (brp && 
		    ((brp->br_reason == FC_BA_RJT_UNABLE) ||
		     (brp->br_reason == FC_BA_RJT_LOG_ERR)))
			break;
		/* fall thru */
	default:
		/*
...
#if defined(__VMKLNX__)
	int_to_scsilun_with_sec_lun_id(fsp->cmd->device->lun,
		       (struct scsi_lun *)fsp->cdb_cmd.fc_lun,
			vmklnx_scsi_cmd_get_secondlevel_lun_id(fsp->cmd));
#else
...
#if defined(__VMKLNX__)
	int_to_scsilun_with_sec_lun_id(lun,
		(struct scsi_lun *)fsp->cdb_cmd.fc_lun,
		vmklnx_scsi_cmd_get_secondlevel_lun_id(fsp->cmd));
#else
...
static void fc_fcp_cleanup(struct fc_lport *lport)
{
	 ...
	 */
	fc_fcp_cleanup_each_cmd(lport, -1, -1, FC_TIMED_OUT);
#else /* defined(__VMKLNX__) */
...
static void fc_fcp_timeout(unsigned long data)
{
	...
	FC_FCP_DBG(fsp, "Handling fc fcp timeout \n");
...
static void fc_fcp_recovery(struct fc_fcp_pkt *fsp, u8 code)
{
	 ...
	 * scsi-ml escalate.
	 */
	FC_FCP_DBG(fsp, "Sending ABORT for status code %d \n", code);
	fc_fcp_send_abort(fsp);
}
...
static void fc_io_compl(struct fc_fcp_pkt *fsp)
{
	...
	case FC_COMPLETE:
		...
		FC_FCP_DBG(fsp, "Returning DID_OK to scsi-ml "
				"due to FC_COMPLETE \n");
		break;
	case FC_ERROR:
#if defined(__VMKLNX__)
	...
	FC_FCP_DBG(fsp, "SCSI cmd=%x, result=%x status code x%x\n",
			sc_cmd->cmnd[0], sc_cmd->result, fsp->status_code );
...
int fc_eh_abort(struct scsi_cmnd *sc_cmd)
{
	...
	if (fc_fcp_lock_pkt(fsp)) {
        ...
        rval = fc_remote_port_chkready(rport);
#endif /* defined(__VMKLNX__) */
        if (rval == (DID_NO_CONNECT << 16)) {
               /* remote port is disconnected */
               fsp->state |= FC_SRB_ABORTED;
               if (fsp->state & FC_SRB_ABORT_PENDING)
                       fsp->state &= ~FC_SRB_ABORT_PENDING;
               fc_fcp_complete_locked(fsp);
