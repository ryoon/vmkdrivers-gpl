 ...
 *
 * ESX Deviation Notes:
 * In ESX the default devloss tmo is set to 10.
 */
/* _VMKLNX_CODECHECK_: fc_set_rport_loss_tmo */
void fc_set_rport_loss_tmo(struct fc_rport *rport, u32 timeout)
{
	if (timeout)
		...
		/* ESX requires dev_loss_tmo value to be 10sec by default */
		rport->dev_loss_tmo = 10;
...
#else
		if (rport_ops && rport_ops->event_callback) {
			FC_RPORT_DBG(rdata, "rport_ops callback ev %d\n", event);
			rport_ops->event_callback(lport, rdata, event);
		}
#endif
 ...
 */
/* _VMKLNX_CODECHECK_: fc_rport_init */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_rport_terminate_io */
