/* ****************************************************************
 * Portions Copyright 2010-2013 VMware, Inc.
 * ****************************************************************/
/*
...
#define fcoe_version_str "1.0.29.9.3-0vmw"
				       ...
				       struct device *parent, int npiv)
{
	...
	shost = lport->host;
#if defined(__VMKLNX__)
	if (vmklnx_scsi_host_set_capabilities(shost,
					      SHOST_CAP_SECONDLEVEL_ADDRESSING)) {
		FCOE_NETDEV_DBG(netdev, "Failed to set capability:0x%x for "
				"adapter\n", SHOST_CAP_SECONDLEVEL_ADDRESSING);
        }
#endif /* defined(__VMKLNX__) */
	port = lport_priv(lport);
	...
	/*
	 * Current FCoE sector size 0xFFFF (65535) seems too much
	 * for VN2VN enabled target to handle. Hence limit transfer
	 * size to 128k so set max sectors to 256.
	 */
	if (netdev->features & NETIF_F_CNA_VN2VN) {
	...
	port->dummy_dev.parent = &fcoe_netdev(lport)->pdev->dev;
	lport->host->dma_boundary = fcoe_netdev(lport)->pdev->dma_mask;
...
int fcoe_reset(struct Scsi_Host *shost)
{
	...
	FCOE_NETDEV_DBG(fcoe->netdev, "fcoe_reset");
