...
#define FCOE_MAX_XID		0xEFFF	/* the max xid supported by fcoe_sw */
...
#if defined (__VMKLNX__)
#define FCOE_LKST_LOGGING   0x04 /* Link state event logging */
extern vmk_LogComponent fcoeLog;
#define FCOE_CHECK_LOGGING(LEVEL, CMD)					\
do {                                                            	\
	if (unlikely(vmk_LogGetCurrentLogLevel(fcoeLog) & LEVEL))	\
		do {							\
			CMD;						\
		} while (0);						\
} while (0)
...
#if defined (__VMKLNX__)
#define FCOE_LKST_DBG(fmt, args...)					\
	FCOE_CHECK_LOGGING(FCOE_LKST_LOGGING,				\
			   printk(KERN_INFO "fcoe: " fmt, ##args);)
#endif /* defined(__VMKLNX__) */
...
{
#if defined(__VMKLNX__)
	if (!lport || !((struct fcoe_port *)lport_priv(lport))->fcoe)
		return NULL;
#endif /* defined(__VMKLNX__) */
