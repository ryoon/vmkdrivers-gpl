...
#if defined(__VMKLNX__)
	/* 2008, 2009: Update from Linux source */
	unsigned int	is_managed:1;	/* device managed by devres management APIs */
...
struct pci_dev *pci_find_slot (unsigned int domain, unsigned int bus, unsigned int devfn);
...
static inline struct pci_dev *pci_find_slot(unsigned int domain, unsigned int bus, unsigned int devfn)
{ return NULL; }
 ...
 *
 *  RETURN VALUE:
 *  Unsigned long value of the start of the BAR
 *
 */
 ...
 *
 *  RETURN VALUE:
 *  Unsigned long value of the end of the BAR
 *
 */
