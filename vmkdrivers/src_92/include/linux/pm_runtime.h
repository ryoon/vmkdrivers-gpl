...
{
	return dev->power2->ignore_children
		|| !atomic_read(&dev->power2->child_count);
}

static inline void pm_suspend_ignore_children(struct device *dev, bool enable)
{
	dev->power2->ignore_children = enable;
}
...
{
	atomic_add_unless(&dev->power2->usage_count, -1, 0);
}

static inline bool device_run_wake(struct device *dev)
{
	return dev->power2->run_wake;
}

static inline void device_set_run_wake(struct device *dev, bool enable)
{
	dev->power2->run_wake = enable;
}

static inline bool pm_runtime_suspended(struct device *dev)
{
	return dev->power2->runtime_status == RPM_SUSPENDED;
}

#else /* !CONFIG_PM_RUNTIME */
#if defined(__VMKLNX__)
#error "The dummy versions of these functions should never be used in ESX"
#endif
