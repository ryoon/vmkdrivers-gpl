...
#ifndef VMKLNX_FCOE_FUTURE
#define VMKLNX_FCOE_FUTURE
#endif /* ifndef VMKLNX_FCOE_FUTURE */
...
/* If secondary lun is valid then update it */
#if defined(__VMKLNX__)
/* _VMKLNX_CODECHECK_: int_to_scsilun_with_sec_lun_id */
static inline void
int_to_scsilun_with_sec_lun_id(unsigned int lun, struct scsi_lun *scsi_lun, uint64_t sllid)
{
 ...
 * @flags:      REC, RETRY and CONF_REQ supported flags
...
struct fc_fcp_pkt {
	...
	struct scatterlist sg; //used when scsi command's use_sg = 0.
#if defined(__VMKLNX__)
        /*
         * we use this to hold the single vmk sg element needed for
         * for the above 'sg' scatterlist struct.
         */
	vmk_sgelem	  vmksgel;
#endif /* defined(__VMKLNX__) */
};
...
#if defined(__VMKLNX__)
	/*
	 * Interface to get associated CNA net_device of the lport
	 *
	 * STATUS: REQUIRED only for CNA drivers that depends on libfcoe for
	 * FIP discovery
	 */
	struct net_device *(*get_cna_netdev) (const struct fc_lport *);
#endif /* defined(__VMKLNX__) */
};
 ...
 * @wwnn:  The new WWNN
 */
/* _VMKLNX_CODECHECK_: fc_set_wwnn */
static inline void fc_set_wwnn(struct fc_lport *lport, u64 wwnn)
{
	...
	if ((lport->host != NULL) &&
	    ((fc_host = shost_to_fc_host(lport->host)) != NULL)) {
		fc_host->node_name = wwnn;
	}
#endif /* defined(__VMKLNX__) */
 ...
 * @wwpn:  The new WWPN
 */
/* _VMKLNX_CODECHECK_: fc_set_wwpn */
 ...
 * @state: The new state
 */
/* _VMKLNX_CODECHECK_: fc_lport_state_enter */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_lport_init_stats */
static inline int fc_lport_init_stats(struct fc_lport *lport)
{
	lport->dev_stats = fcoe_alloc_percpu(sizeof(struct fcoe_dev_stats), 
					__alignof__(struct fcoe_dev_stats));
	if (!lport->dev_stats)
		return -ENOMEM;
	return 0;
}
 ...
 */
/* _VMKLNX_CODECHECK_: fc_lport_free_stats */
 ...
 * Returns: libfc lport
 */
/* _VMKLNX_CODECHECK_: libfc_host_alloc */
 ...
 *****************************/
/* _VMKLNX_CODECHECK_: fc_fcp_is_read */
...
#if defined(__VMKLNX__)
int fc_slave_configure(struct scsi_device *);
