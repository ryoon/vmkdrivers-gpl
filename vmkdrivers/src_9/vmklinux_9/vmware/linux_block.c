                         ...
                         uint16_t byteOffset)
{
   ...
   sense->sksv = VMK_TRUE;     // sense key specific data is valid
   sense->cd = isCommand;
   sense->epos = vmk_CPUToBE16(byteOffset);
}
                          ...
                          vmk_ScsiDeviceStatus device)
{
   ...
   vmk_TimerAdd((vmk_TimerCallback) LinuxBlockCompleteCommand,
                (void *) cmd, 0, VMK_FALSE,
...
retry:
   ...
   vmk_NameFormat(&adapter->blockDevName, "%s", name);
