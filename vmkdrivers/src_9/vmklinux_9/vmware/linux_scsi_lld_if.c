...
vmklnx_setup_adapter_tls(struct vmklnx_ScsiAdapter *vmklnx26ScsiAdapter)
{
   ...
   numCompObjects = vmk_ScsiGetNumCompObjects(vmkAdapter);
...
scsi_add_host(struct Scsi_Host *sh, struct device *pDev)
{
   ...
   vmklnx26ScsiAdapter = 
   (struct vmklnx_ScsiAdapter *)kzalloc(sizeof(*vmklnx26ScsiAdapter), GFP_KERNEL);
   ...
   if (pDev->dev_type == PCI_DEVICE_TYPE) {
      ...
      if (LinuxPCI_DeviceIsPAECapable(pciDev)) {
      ...
      status = vmk_PCIGetGenDevice(pciDevice, &vmkAdapter->device);
      VMK_ASSERT(status == VMK_OK);
 ...
 * 
 * \param sh, Maximum queues adapter can create
 ...
 *      be created in adapter
 * \sa None
 **********************************************************************
 */
int
vmklnx_scsi_get_num_ioqueue(struct Scsi_Host *sh,
                            unsigned int maxQueue)
{
    struct vmklnx_ScsiAdapter *adapter = sh->adapter;
    int num_queues;
 
    num_queues = vmk_ScsiGetNumCompObjects(adapter->vmkAdapter);
