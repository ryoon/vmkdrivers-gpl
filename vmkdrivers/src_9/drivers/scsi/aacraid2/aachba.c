...
int aac_get_adapter_info(struct aac_dev* dev)
{
...
#if !defined(__VMKLNX__)
	/*
	 * avoid calling get_fs which is currently unimplemeneted in vmklinux.
	 */  
#ifndef MODULE
	...
	if( (sizeof(dma_addr_t) > 4) && (num_physpages > (0xFFFFFFFFULL >> PAGE_SHIFT)) && (dev->adapter_info.options & AAC_OPT_SGMAP_HOST64)){
	    ...
	    * number of physical pages on the system. vmklinux does not
...
#if defined(__VMKLNX__)
	/*
	 * Re-apply CS182428: adjusting queue depth to 2 may introduce
	 * some problems in ESX and will generate a lot log spew.
	 */
	return;
#else
...
int aac_scsi_cmd(struct scsi_cmnd * scsicmd)
{
			...
			break;
	{
		...
		if (sizeof(cp) < alloc_len) {
			 ...
			 */
			char *buf = kzalloc(alloc_len, GFP_ATOMIC);
			if (buf) {
...
#if !defined(__VMKLNX__)
			/*
			 * This code was removed because in vmklinux,
...
#if defined(__VMKLNX__)
		/*
		 * PR570447: Removing the CHECK_CONDITION flag for sanity.
		 */
...
#if defined(__VMKLNX__)
		/*
		* PR570447: In CHECK_CONDITION status, the driver should return DID_OK.
		*/
		scsicmd->result = DID_OK << 16 | COMMAND_COMPLETE << 8;
#endif /* defined (__VMKLNX__) */
...
#if defined (__VMKLNX__)
	if (unlikely((scsicmd->result & 0x000000FF) == SAM_STAT_CHECK_CONDITION && ((scsicmd->result & 0x00FF0000) >> 16) == DID_ERROR)) {
		/*
		 * PR570447: Double check the return status for sanity
		 * Clear the HOST STATUS bit and set to DID_OK.
		 * This makes sure that when CHECK_CONDITION is set, the host status returned to upper layer is DID_OK.
		 */
		printk(KERN_WARNING "aac_srb_callback: Command 0x%x error: DID_ERROR with Check Condition set. Resetting to DID_OK.\n",
			scsicmd->cmnd[0]);
		scsicmd->result &= ~0x00FF0000;
		scsicmd->result |= DID_OK << 16;
	}
#endif /* defined (__VMKLNX__) */
	aac_fib_complete(fibptr);
