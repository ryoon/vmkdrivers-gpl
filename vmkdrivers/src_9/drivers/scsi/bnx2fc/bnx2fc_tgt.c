...
void bnx2fc_flush_active_ios(struct bnx2fc_rport *tgt)
{
	...
	bnx2fc_dbg(LOG_IOERR, "Entered flush_active_ios - %d, port_id = 0x%x\n",
		tgt->num_active_ios.counter, tgt->rdata->ids.port_id);
	...
	list_for_each_safe(list, tmp, &tgt->active_cmd_queue) {
		...
		bnx2fc_dbg(LOG_IOERR, "cmd_queue cleanup - xid = 0x%x\n", io_req->xid);
	...
	list_for_each_safe(list, tmp, &tgt->active_tm_queue) {
		...
		bnx2fc_dbg(LOG_IOERR, "tm_queue cleanup\n");
			   ...
			   struct fc_rport_priv *rdata)
{
	...
	if (hba->num_ofld_sess >= BNX2FC_NUM_MAX_SESS) {
		bnx2fc_dbg(LOG_IOERR, "exceeded max sessions. logoff this tgt\n");
		tgt->fcoe_conn_id = -1;
		return -1;
	}
					...
					struct bnx2fc_rport *tgt)
{
	...
	tgt->sq = bnx2fc_alloc_dma(hba, tgt->sq_mem_size,
					&tgt->sq_dma);
	if (!tgt->sq) {
	...
	tgt->cq = bnx2fc_alloc_dma(hba, tgt->cq_mem_size,
					&tgt->cq_dma);
	if (!tgt->cq) {
	...
	tgt->rq = bnx2fc_alloc_dma(hba, tgt->rq_mem_size,
					&tgt->rq_dma);
	if (!tgt->rq) {
	...
	tgt->rq_pbl = bnx2fc_alloc_dma(hba, tgt->rq_pbl_size,
						&tgt->rq_pbl_dma);
	if (!tgt->rq_pbl) {
	...
	tgt->xferq = bnx2fc_alloc_dma(hba, tgt->xferq_mem_size,
						&tgt->xferq_dma);
	if (!tgt->xferq) {
	...
	tgt->confq = bnx2fc_alloc_dma(hba, tgt->confq_mem_size,
						&tgt->confq_dma);
	if (!tgt->confq) {
	...
	tgt->confq_pbl = bnx2fc_alloc_dma(hba, tgt->confq_pbl_size,
					      &tgt->confq_pbl_dma);
	if (!tgt->confq_pbl) {
	...
	tgt->conn_db = bnx2fc_alloc_dma(hba, tgt->conn_db_mem_size,
						&tgt->conn_db_dma);
	if (!tgt->conn_db) {
	...
	tgt->lcq = bnx2fc_alloc_dma(hba, tgt->lcq_mem_size,
					&tgt->lcq_dma);
						...
						struct bnx2fc_rport *tgt)
{
	...
	/* Free LCQ */
	if (tgt->lcq) {
		bnx2fc_free_dma(hba, tgt->lcq_mem_size,
				    tgt->lcq, tgt->lcq_dma);
		tgt->lcq = NULL;
	}
	/* Free connDB */
	if (tgt->conn_db) {
		...
		bnx2fc_free_dma(hba, tgt->conn_db_mem_size,
				    tgt->conn_db, tgt->conn_db_dma);
		tgt->conn_db = NULL;
	}
	/* Free confq  and confq pbl */
	if (tgt->confq_pbl) {
		bnx2fc_free_dma(hba, tgt->confq_pbl_size,
				    tgt->confq_pbl, tgt->confq_pbl_dma);
		tgt->confq_pbl = NULL;
	}
	if (tgt->confq) {
		bnx2fc_free_dma(hba, tgt->confq_mem_size,
				    tgt->confq, tgt->confq_dma);
		tgt->confq = NULL;
	}
	/* Free XFERQ */
	if (tgt->xferq) {
		bnx2fc_free_dma(hba, tgt->xferq_mem_size,
				    tgt->xferq, tgt->xferq_dma);
		tgt->xferq = NULL;
	}
	/* Free RQ PBL and RQ */
	if (tgt->rq_pbl) {
		bnx2fc_free_dma(hba, tgt->rq_pbl_size,
				    tgt->rq_pbl, tgt->rq_pbl_dma);
		tgt->rq_pbl = NULL;
	}
	if (tgt->rq) {
		bnx2fc_free_dma(hba, tgt->rq_mem_size,
				    tgt->rq, tgt->rq_dma);
		tgt->rq = NULL;
	}
	/* Free CQ */
	if (tgt->cq) {
		bnx2fc_free_dma(hba, tgt->cq_mem_size,
				    tgt->cq, tgt->cq_dma);
		tgt->cq = NULL;
	}
	/* Free SQ */
	if (tgt->sq) {
		bnx2fc_free_dma(hba, tgt->sq_mem_size,
				    tgt->sq, tgt->sq_dma);
		tgt->sq = NULL;
	}
