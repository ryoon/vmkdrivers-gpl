...
static void bnx2fc_cmd_timeout(struct work_struct *work)
{
	...
	case BNX2FC_SCSI_CMD:
		if (test_and_clear_bit(BNX2FC_FLAG_EH_ABORT, &io_req->req_flags)) {
			...
			bnx2fc_dbg(LOG_IOERR, "ABTS for IO with oxid = 0x%x timed out \n",
					io_req->xid);
						...
						u16 min_xid, u16 max_xid)
{
	...
	for (i = 0; i < num_ios; i++) {
		...
		bdt_info->bd_tbl = bnx2fc_alloc_dma_bd(hba,
						      bd_tbl_sz,
						      &bdt_info->bd_tbl_dma);
		if (!bdt_info->bd_tbl) {
			printk(KERN_ERR PFX "failed to alloc "
...
int bnx2fc_init_mp_req(struct bnx2fc_cmd *io_req)
{
	...
	mp_req->req_buf = bnx2fc_alloc_dma(hba, PAGE_SIZE,
					       &mp_req->req_buf_dma);
	if (!mp_req->req_buf) {
	...
	mp_req->resp_buf = bnx2fc_alloc_dma(hba, PAGE_SIZE,
					       &mp_req->resp_buf_dma);
	if (!mp_req->resp_buf) {
	...
	mp_req->mp_req_bd = bnx2fc_alloc_dma(hba, sz,
						 &mp_req->mp_req_bd_dma);
	if (!mp_req->mp_req_bd) {
	...
	mp_req->mp_resp_bd = bnx2fc_alloc_dma(hba, sz,
						 &mp_req->mp_resp_bd_dma);
	if (!mp_req->mp_resp_bd) {
...
static int bnx2fc_execute_tmf(struct bnx2fc_rport *tgt, struct scsi_cmnd *sc_cmd,
				 u8 tm_flags)
{
	struct bnx2fc_cmd *io_req;
	...
	if (!tgt || !sc_cmd) {
		bnx2fc_dbg(LOG_IOERR, "execute_tmf invalid arg\n");
		return FAILED;
	}

	if (!(test_bit(BNX2FC_FLAG_SESSION_READY, &tgt->flags))) {
		printk(KERN_ERR PFX "execute_tmf: tgt not offloaded\n");
		rc = FAILED;
		goto reset_tmf_err;
	}
	...
	bnx2fc_dbg(LOG_IOERR, "execute_tmf: conn:%d tm_flags = %d\n",
			tgt->fcoe_conn_id, tm_flags);
...
retry_tmf:
	...
	task_page = (struct fcoe_task_ctx_entry *) port->hba->task_ctx[task_idx];
...
static int bnx2fc_initiate_tmf(struct scsi_cmnd *sc_cmd, u8 tm_flags)
{
...
#ifdef __VMKLNX__
	rval = bnx2fc_remote_port_chkready(rport);
#else
	rval = fc_remote_port_chkready(rport);
#endif
	if (rval) {
		BNX2FC_ERR("device_reset rport not ready\n");
		rc = FAILED;
		goto tmf_err;
	}
	...
	rc = bnx2fc_execute_tmf(tgt, sc_cmd, tm_flags);
...
int bnx2fc_initiate_abts(struct bnx2fc_cmd *io_req)
{
	...
	bnx2fc_dbg(LOG_IOERR, "Entered ox_id 0x%x\n", io_req->xid);
...
int bnx2fc_initiate_cleanup(struct bnx2fc_cmd *io_req)
{
	...
	bnx2fc_dbg(LOG_IOERR, "Entered xid = 0x%x\n", io_req->xid);
...
int bnx2fc_eh_device_reset(struct scsi_cmnd *sc_cmd)
{
	...
	if (sc_cmd->vmkflags & VMK_FLAGS_USE_LUNRESET) {
		/* LUN reset */
		printk("LUN RESET requested\n");
		tm_flags = FCP_TMF_LUN_RESET;
	} else {
		/* TARGET reset */
		printk("TARGET RESET requested\n");
	...
	rc = bnx2fc_initiate_tmf(sc_cmd, tm_flags);
...
int bnx2fc_expl_logo(struct fc_lport *lport, struct bnx2fc_cmd *io_req)
{
	...
	bnx2fc_dbg(LOG_ERROR, "Expl logo - tgt flags = 0x%lx\n", tgt->flags);
	...
	ret = wait_for_completion_timeout(&io_req->tm_done, (2 * HZ));
	...
	spin_lock_bh(&tgt->tgt_lock);
	if (!ret) {
		BNX2FC_ERR("expl_logo: timed out- missed cleanup\n");
	}
...
int bnx2fc_eh_bus_reset(struct scsi_cmnd *sc_cmd)
{
	...
	int rc = SUCCESS;
	int rval, i;
	u8 tm_flags = FCP_TMF_TGT_RESET;

	lport = shost_priv(host);
	port = lport_priv(lport);
	hba = port->hba;

	bnx2fc_dbg(LOG_IOERR, "Entered\n");

	mutex_lock(&hba->hba_mutex); /* Need to check this */

	for (i = 0; i < BNX2FC_NUM_MAX_SESS; i++) {
		tgt = hba->tgt_ofld_list[i];
		if (!tgt) {
			bnx2fc_dbg(LOG_IOERR, "tgt_ofld_list[%d] Invalid\n", i);
			continue;
		}
		rport = tgt->rport;
#ifdef __VMKLNX__
...
#endif
		if (rval) {
			printk(KERN_ALERT PFX "_bus_reset: rport[%d] not ready\n", 
						tgt->fcoe_conn_id);
			continue;
		}
		if (lport->state != LPORT_ST_READY || !(lport->link_up)) {
			printk(KERN_ERR PFX "bus_reset: link is not ready\n");
			rc = FAILED;
			goto bus_reset_err;
		}
		/* bnx2fc_execute_tmf is a blocking call */
		rc = bnx2fc_execute_tmf(tgt, sc_cmd, tm_flags);
		if (rc != SUCCESS)
			printk(KERN_ALERT PFX "bus_reset: rport[%d] error \n", 
						tgt->fcoe_conn_id);
	}
...
int bnx2fc_eh_abort(struct scsi_cmnd *sc_cmd)
{
...
#ifdef __VMKLNX__
	if (bnx2fc_remote_port_chkready(rport)) {
#else
	...
	if (!io_req) {
		/* Command might have just completed, fail abort request as
 		 * scsi_done() is not called in this context and is expected
 		 * to be issued in IO completion path
 		 */
		printk(KERN_ERR PFX "eh_abort: io_req is NULL\n");
		hba->io_cmpl_abort_race2++;
		spin_unlock_bh(&tgt->tgt_lock);
		return FAILED;
	}
	printk(KERN_ERR PFX "eh_abort: xid = 0x%x \n", io_req->xid);

	if (test_bit(BNX2FC_FLAG_EXPL_LOGO, &tgt->flags)) {
		BNX2FC_ERR("eh_abort: io_req (xid = 0x%x) "
				"already in expl logo processing\n", io_req->xid);
		spin_unlock_bh(&tgt->tgt_lock);
		return rc;
	}
	...
	if (test_bit(BNX2FC_FLAG_IO_COMPL, &io_req->req_flags)) {
		 ...
		 */
		printk(KERN_ERR PFX  "abort succeeded\n");
		rc = SUCCESS;
		bnx2fc_scsi_done(io_req, DID_ABORT);
		kref_put(&io_req->refcount, bnx2fc_cmd_release);
	}
				  ...
				  u8 num_rq)
{
	...
	bnx2fc_dbg(LOG_IOERR, "Entered process_cleanup_compl xid = 0x%x cmd_type = %d\n",
			       ...
			       u8 num_rq)
{
	...
	bnx2fc_dbg(LOG_IOERR, "Entered process_abts_compl xid = 0x%x cmd_type = %d\n",
		   io_req->xid, io_req->cmd_type);
...
static void bnx2fc_lun_reset_cmpl(struct bnx2fc_cmd *io_req)
{
	...
	bnx2fc_dbg(LOG_IOERR, "bnx2fc_lun_reset_cmpl : End\n");
}
			     ...
			     struct fcoe_task_ctx_entry *task, u8 num_rq)
{
	...
	//temp_hdr = (u64 *)&task->cmn.general.comp_info;
				 ...
				 u8 num_rq)
{
	...
	if (num_rq) {
		...
		if ((fcp_rsp_len == 4) || (fcp_rsp_len == 8)) {
			...
			bnx2fc_dbg(LOG_SCSI_TM, "fcp_rsp_code = %d\n",
				   io_req->fcp_rsp_code);
		}
				   ...
				   u8 num_rq)
{
	...
	if (io_req->on_active_queue) {
		 ...
		 */
		bnx2fc_dbg(LOG_IOERR, "xid not on active_cmd_queue\n");
		if (io_req->wait_for_comp)
			if (test_and_clear_bit(BNX2FC_FLAG_EH_ABORT,
					       &io_req->req_flags))
				complete(&io_req->tm_done);
