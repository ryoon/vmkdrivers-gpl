...
#define FCOE_HSI_MAJOR_VERSION (1)	// Implies on a change broken previous HSI
#define FCOE_HSI_MINOR_VERSION (1)	// Implies on a change which does not broken previous HSI
...
#define FCOE_E_D_TOV_SDM_TIMER_RESOLUTION (FCOE_E_D_TOV_TIMER_RESOLUTION_MS * 1000 / 4)
...
#define FCOE_E_D_TOV_DEFAULT_WRAPAROUND_VAL	(2000 / FCOE_E_D_TOV_TIMER_RESOLUTION_MS)

// REC_TOV timer default wraparound value (3 sec) in 20 ms resolution
#define FCOE_REC_TOV_DEFAULT_WRAPAROUND_VAL	(3000 / FCOE_REC_TIMER_RESOLUTION_MS)
...
#define	FCOE_TASK_TX_STATE_NORMAL						0	// In case timer services are required then shall be updated by Xstorm after start processing the task. In case no timer facilities are required then the driver would initialize the state to this value
#define	FCOE_TASK_TX_STATE_INIT							1	// After driver has initialize the task in case timer services required
************************************** Remove FCP_CMD write tce sleep                *****************************************************************/
#define	FCOE_TASK_TX_STATE_INIT                                     0	// After driver has initialize the task in case timer services required
#define	FCOE_TASK_TX_STATE_NORMAL                                   1	// In case timer services are required then shall be updated by Xstorm after start processing the task. In case no timer facilities are required then the driver would initialize the state to this value
#define	FCOE_TASK_TX_STATE_ABORT                                    2	// Task is under abort procedure. Updated in order to stop processing of pending WQEs on this task
#define	FCOE_TASK_TX_STATE_ERROR                                    3	// For E_D_T_TOV timer expiration in Xstorm (Class 2 only)
#define	FCOE_TASK_TX_STATE_WARNING                                  4	// For REC_TOV timer expiration indication received from Xstorm
#define	FCOE_TASK_TX_STATE_UNSOLICITED_COMPLETED                    5	// For completed unsolicited task
#define	FCOE_TASK_TX_STATE_EXCHANGE_CLEANUP                         6	// For exchange cleanup request task
#define	FCOE_TASK_TX_STATE_SEQUENCE_CLEANUP                         7	// For sequence cleanup request task
#define	FCOE_TASK_TX_STATE_ABTS_TX                                  8	// For completion the ABTS task.

#define	FCOE_TASK_RX_STATE_NORMAL						0
#define	FCOE_TASK_RX_STATE_COMPLETED					1
#define	FCOE_TASK_RX_STATE_INTER_COMP					2	// Obsolete: Intermediate completion (middle path with local completion)
#define	FCOE_TASK_RX_STATE_WARNING						3	// For REC_TOV timer expiration indication received from Xstorm
#define	FCOE_TASK_RX_STATE_ERROR						4	// For E_D_T_TOV timer expiration in Ustorm
#define	FCOE_TASK_RX_STATE_ABTS_IN_PROCESS				5	// FW only: First visit at rx-path, part of the abts round trip 
#define	FCOE_TASK_RX_STATE_ABTS_TRANSMITTED				6	// FW only: Second visit at rx-path, after ABTS frame transmitted
#define FCOE_TASK_RX_STATE_ABTS_COMPLETED				7	// Special completion indication in case of task was aborted.
#define	FCOE_TASK_RX_STATE_EXCHANGE_CLEANUP_IN_PROCESS  8	// FW only: First visit at rx-path, part of the cleanup round trip 
#define FCOE_TASK_RX_STATE_EXCHANGE_CLEANUP_COMPLETED	9	// FW only: Special completion indication in case of task was cleaned.
#define FCOE_TASK_RX_STATE_ABORT_CLEANUP_COMPLETED		10	// Not in used: Special completion indication (in task requested the exchange cleanup) in case cleaned task is in non-valid.
#define FCOE_TASK_RX_STATE_IGNORED_SEQUENCE_CLEANUP		11	// Special completion indication (in task requested the sequence cleanup) in case cleaned task was already returned to normal.
...
#define FCOE_HALF_ELEMENTS_IN_HASH_TABLE_ROW	(FCOE_MAX_ELEMENTS_IN_HASH_TABLE_ROW / 2)
...
#endif /*__57XX_FCOE_CONSTANTS_H_ */
