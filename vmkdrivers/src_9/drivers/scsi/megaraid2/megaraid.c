...
#define MEGARAID_MODULE_VERSION "2.00.4-9vmw"
...
mega_build_cmd(adapter_t *adapter, Scsi_Cmnd *cmd, int *busy)
{
	...
	if(islogical) {
		...
		case MODE_SENSE: {
#if defined(__VMKLNX__)
			scsi_sg_memset(cmd, 0, cmd->cmnd[4]);
...
static inline int mega_vmware_chk_phys(Scsi_Cmnd * SCpnt, int islogical)
{
        ...
        /*
         * PR570447
         * Returning BAD_TARGET breaks VMware Pluggable Storage Architecture
         * (PSA) assumptions. BAD_TARGET should only be returned when the
         * target is out of range or otherwise truly invalid, not just offline
         * or not reporting. Need to return VMK_SCSI_HOST_NO_CONNECT instead
         * of DID_BAD_TARGET in such cases.
         */
...
mega_cmd_done(adapter_t *adapter, u8 completed[], int nstatus, int status)
{
	...
	for( i = 0; i < nstatus; i++ ) {
		...
		if( cmdid == CMDID_INT_CMDS ) { /* internal command */
			...
			/*
			 * pr# 410322
			 * megaraid_abort won't abort cmds that are already sent to
			 * f/w, it just returns FAILED. Since f/w completed that cmd,
			 * just let usual completion process happen for such cmds
			 * instead of returning DID_ABORT!
			 */
...
#if !defined(__VMKLNX__)
			/*
			 * See comment above - same applies here too!
			 */
		...
		case 0x08:	/* ERR_DEST_DRIVE_FAILED, i.e.
			...
			break;
				...
				/* 
				 *  PR 563269 and 575121: default catch all
				 * other INQUIRY commands with 0x1 status to
				 * return DID_OK with Check Condition
				 */
				printk("INQUIRY with EVPD 0x%x, page 0x%x set to CC 0x5/0x24/0x00\n",
					cmd->cmnd[1],
					cmd->cmnd[2]);
				cmd->result = (DRIVER_SENSE << 24) | (DID_OK << 16) |
					(CHECK_CONDITION << 1);
				cmd->resid = cmd->request_bufflen;
				scsi_build_sense_buffer(0x0, cmd->sense_buffer,
							ILLEGAL_REQUEST, 0x24, 0x00);
				break;
			}
                        /* Fall through ... */
#endif /* defined(__VMKLNX__) */
		...
		default:
...
#if defined(__VMKLNX__)
			{
				/*
                                 * PR570447
				 * Returning BAD_TARGET breaks VMware
				 * Pluggable Storage Architecture (PSA)
				 * assumptions. BAD_TARGET should only be
				 * returned when the target is out of range or
				 * otherwise truly invalid, not just offline
				 * or not reporting. Need to return
				 * VMK_SCSI_HOST_NO_CONNECT instead of
				 * DID_BAD_TARGET in such cases.
				 */
				cmd->result |= (DID_NO_CONNECT << 16)|status;
			}
#else
...
mega_build_sglist(adapter_t *adapter, scb_t *scb, u32 *buf, u32 *len)
{
	...
	*len = 0;
	...
	scsi_for_each_sg(cmd, sgl, sgcnt, idx) {
#else /* !defined(__VMKLNX__) */
...
#if defined(__VMKLNX__)
	/* do reset again - for SG_VMK type */
	sg_reset(sgl);
#endif /* defined(__VMKLNX__) */
...
megaraid_abort(Scsi_Cmnd *cmd)
{
	...
	/*
	 * pr# 371149
	 * Cmd completions from interrupt handler can modify adpater
	 * lists while same lists can be modified here too, so take
	 * adpater lock.
	 */
	...
	spin_unlock_irq(&adapter->lock);
        ...
        return rval == TRUE ? SUCCESS : FAILED;
#else /* !defined(__VMKLNX__) */
	rval =  megaraid_abort_and_reset(adapter, cmd, SCB_ABORT);
...
#if defined(__VMKLNX__)
#define MEGA_RESET_WAIT_TIME    180     /* 3mins */
...
megaraid_reset(struct scsi_cmnd *cmd)
{
	...
	printk(KERN_WARNING "megaraid: Resetting <c=%d t=%d l=%d>\n",
	...
	/*
	 * First clean up all the cmds owned by driver
	 * from pending list
	 */
	list_for_each_safe(pos, next, &adapter->pending_list) {
		scb = list_entry(pos, scb_t, list);
		...
		if(scb->state & SCB_ISSUED ) {
			/* skip cmds that are issued to firmware */
			continue;
		} else {
			/* Mark the cmd carried by scb to DID_RESET, and put the cmd to completed_list */
			scb->cmd->result = (DID_RESET << 16);
			list_add_tail(SCSI_LIST(scb->cmd), &adapter->completed_list);

			/* Free up scb AFTER carried cmd is moved to completed_list*/
			mega_free_scb(adapter, scb);
		}
	}
	...
	/*
	 * Now wait for the cmds issued to firmware to complete
	 */
	start = jiffies;
	while (atomic_read(&adapter->pend_cmds) > 0 ||
	       !list_empty(&adapter->pending_list)) {
		if (time_after(jiffies, start + MEGA_RESET_WAIT_TIME * HZ)) {
			rval = FAILED;	/* cmds still not completed! */
			break;
		}
		/* TODO: release lock and acquire after sleep */
		msleep(1000);
	}
	...
	printk(KERN_WARNING "megaraid: Reset %s for <c=%d t=%d l=%d>\n",
			(rval == SUCCESS) ? "SUCCESS" : "FAILED",
                        cmd->device->channel, cmd->device->id, cmd->device->lun);
 ...
 * XXX: PR 632936
 *
 * This data structure needs to be larger than a LinuxPCIDevExt.
 */
typedef struct {
...
{
#if defined(__VMKLNX__)
        ExtendedPciDevice *pciDevExt;

        pciDevExt = kzalloc(sizeof(ExtendedPciDevice), GFP_KERNEL);
        if (pciDevExt == NULL) {
                return -1;
        }
        *pdev = &pciDevExt->linuxDev;
#else /* !defined(__VMKLNX__) */
        *pdev = alloc_pci_dev();
...
{
        ExtendedPciDevice *pciDevExt = container_of(pdev, ExtendedPciDevice, linuxDev);
...
megaraid_probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
...
#if defined(__VMKLNX__)
	printk(KERN_NOTICE "megaraid: found 0x%4.04x:0x%4.04x:domain %04x:",
                id->vendor, id->device, pci_domain);
	printk("bus %d:slot %d:func %d\n",
		pci_bus, PCI_SLOT(pci_dev_func), PCI_FUNC(pci_dev_func));
#else /* !defined(__VMKLNX__) */
	...
	if (error)
		goto out_free_mbox;
#if defined(__VMKLNX__)
	vmklnx_scsi_register_poll_handler(host, adapter->host->irq, (adapter->flag & BOARD_MEMMAP) ?
					  megaraid_isr_memmapped : megaraid_isr_iomapped,
					  adapter);
#endif /* defined(__VMKLNX__) */
