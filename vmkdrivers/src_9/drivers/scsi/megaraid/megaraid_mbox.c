...
int __init mraid_mm_init(void);
void __exit mraid_mm_exit(void);
...
#if defined(__VMKLNX__)
	vmklnx_scsi_register_poll_handler(host, adapter->irq, megaraid_isr, adapter);
#endif /* defined(__VMKLNX__) */
	scsi_scan_host(host);
...
megaraid_mbox_build_cmd(adapter_t *adapter, struct scsi_cmnd *scp, int *busy)
{
	...
	if (islogical) {
		...
		case MODE_SENSE:
#if defined(__VMKLNX__)
			scsi_sg_memset(scp, 0, scp->cmnd[4]);
#else /* !defined(__VMKLNX__) */
			if (scp->use_sg) {
				struct scatterlist	*sgl;
				caddr_t			vaddr;
