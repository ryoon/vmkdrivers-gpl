...
#if defined(__VMKLNX__)
		if (phba->sli4_hba.pc_sli4_params.featurelevel_1 ==
		    LPFC_SLI_INTF_FEATURELEVEL1_1) {
			shost->max_sectors = MAX_SECTORS_TIGERSHARK;
		}
#endif
...
/**
 * lpfc_sli4_queues_reduce - Reduce the total number of SLI4 queues
 * @phba: pointer to lpfc hba data structure.
 * @cnt: number of queues to release
 *
 * This routine is invoked to reduce the number of WQs, CQs, and EQs
 * to make them consistent with the MQ API expectations.  The caller
 * passes the @phba and @cnt.  The driver releases the last @cnt queues
 * defined in the phba.
 *
 * Return codes
 *      0 - sucessful
 ...
 **/
static void
lpfc_sli4_queues_reduce(struct lpfc_hba *phba, uint32_t cnt)
{
	int fcp_qidx;

	/* Reduce the FCP work queue */
	for (fcp_qidx = phba->cfg_fcp_wq_count - cnt;
	     fcp_qidx < phba->cfg_fcp_wq_count;
	     fcp_qidx++) {
		lpfc_sli4_queue_free(phba->sli4_hba.fcp_wq[fcp_qidx]);
	}
	/* Reduce FCP response complete queue */
	for (fcp_qidx = phba->cfg_fcp_eq_count - cnt;
	     fcp_qidx < phba->cfg_fcp_eq_count;
	     fcp_qidx++) {
		lpfc_sli4_queue_free(phba->sli4_hba.fcp_cq[fcp_qidx]);
	}
	/* Reduce fast-path event queue */
	for (fcp_qidx = phba->cfg_fcp_eq_count - cnt;
	     fcp_qidx < phba->cfg_fcp_eq_count;
	     fcp_qidx++) {
...
lpfc_create_shost(struct lpfc_hba *phba)
{
	 ...
	 * When enabled, the number of queues needed is a phba consideration
	 * because virtual ports share the physical port resources.
	 */
	if (phba->cfg_use_mq == LPFC_DISABLE_MQ)
		return 0;

	if (lpfc_is_sli4(phba)) {
		/*
		 * Call vmkernel with the maximum possible WQs, not
		 * the default number available.  If the kernel requires
		 * fewer queues, reduce the queue count.
		 */
		phba->mq_num_qs = vmklnx_scsi_get_num_ioqueue(shost,
							      LPFC_FN_WQN_MAX);
		if ((phba->mq_num_qs <= 1) ||
		    (phba->mq_num_qs > phba->cfg_fcp_wq_count)) {
			lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
					"2814 MultiQueue not supportable on "
		...
		if (phba->cfg_fcp_wq_count > phba->mq_num_qs) {
			q_cnt = phba->cfg_fcp_wq_count - phba->mq_num_qs;
			lpfc_printf_log(phba, KERN_INFO, LOG_INIT,
					"2846 Eliminating excess %d queues. "
					"loading driver driver with %d "
					"queues.\n", q_cnt, phba->mq_num_qs);
			lpfc_sli4_queues_reduce(phba, q_cnt);
			phba->cfg_fcp_wq_count -= q_cnt;
			phba->cfg_fcp_eq_count -= q_cnt;
			phba->sli4_hba.cfg_eqn -= q_cnt;
		}

		lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
				"2815 MultiQueue supported with x%x queues "
				" and MSI-X vectors.\n", phba->mq_num_qs);
	}
	return 0;
}
