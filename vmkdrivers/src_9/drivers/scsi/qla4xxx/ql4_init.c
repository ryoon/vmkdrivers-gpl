...
int qla4_is_relogin_allowed(struct scsi_qla_host *ha, uint32_t conn_err)
{
	...
	DEBUG2(dev_info(&ha->pdev->dev,
               "qla4_is_relogin_allowed - conn_err=0x%08x relogin=%d\n",
               conn_err, relogin));
...
static int qla4xxx_build_ddb_list(struct scsi_qla_host *ha)
{
	...
	for (fw_ddb_index = 0; fw_ddb_index < MAX_DDB_ENTRIES;
		...
		if (!(le16_to_cpu(fw_ddb_entry->options) & DDB_OPT_DISC_SESSION) &&
			...
			DEBUG6(dev_info(&ha->pdev->dev, "%s: DDB[%d] osIdx = %d "
	...
	list_for_each_entry(ddb_entry, &ha->ddb_list, list) {
		...
		list_for_each_entry(ddb_entry, &ha->ddb_list, list) {
			if ((strncmp(ddb_entry->iscsi_name, fw_ddb_entry->iscsi_name,
			    ...
			    (ddb_entry->tpgt == fw_ddb_entry->tgt_portal_grp) &&
		...
		if ((atomic_read(&ddb_entry->state) == DDB_STATE_MISSING) ||
			...
			if (ddb_entry->fw_ddb_device_state
				...
				if (qla4xxx_get_fwddb_entry(ha,
				    ...
				    NULL, NULL, &conn_err, NULL, NULL)
				    == QLA_SUCCESS) {
					if (qla4_is_relogin_allowed(ha, conn_err))
						qla4xxx_set_ddb_entry(ha,
						    ddb_entry->fw_ddb_index, 0);
				}
			}
		}
	}
 ...
 * @fw_ddb_index - Firmware's device database index
 * @state - Device state
 * @probe - Detect devices coming online late in probe
 *
 * This routine processes a Decive Database Changed AEN Event.
 **/
int qla4xxx_process_ddb_changed(struct scsi_qla_host *ha, uint32_t fw_ddb_index,
				uint32_t state, uint32_t conn_err, uint32_t probe)
{
	struct ddb_entry * ddb_entry;
	...
	if (ddb_entry->fw_ddb_device_state == DDB_DS_SESSION_ACTIVE) {
		...
		if (atomic_read(&ddb_entry->state) == DDB_STATE_ONLINE) {
			DEBUG6(dev_info(&ha->pdev->dev, "%s mark missing "
				"ddb_entry 0x%p sess 0x%p conn 0x%p\n",
				__func__, ddb_entry,
