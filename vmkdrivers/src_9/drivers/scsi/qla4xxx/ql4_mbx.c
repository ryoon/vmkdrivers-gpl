...
int qla4xxx_initialize_fw_cb(struct scsi_qla_host * ha)
{
	...
	if (gratuitous_arp) {
		init_fw_cb->ipv4_ip_opts |=
			__constant_cpu_to_le16(IPOPT_GRATUITOUS_ARP);
	}
	init_fw_cb->conn_ka_timeout = __constant_cpu_to_le16(ka_timeout);
#endif
			    ...
			    uint16_t *connection_id)
{
	...
	if (fw_ddb_entry) {
		DEBUG6(dev_info(&ha->pdev->dev, "%s: DDB[%d] MB0 %04x Tot %d "
