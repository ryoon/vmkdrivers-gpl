...
void qla4xxx_print_scsi_cmd(struct scsi_cmnd *cmd)
{
	...
	if (cmd->use_sg) {
		...
		/*
		 * This dumps vmk_sgelem structs of an cmd
		 */
		qla4xxx_dump_buffer((caddr_t) sg->cursgel,
				    (cmd->use_sg * sizeof(*(sg->cursgel))));
...
void
qla4xxx_dump_buffer(void *buffer, uint32_t size)
{
	uint32_t i;
