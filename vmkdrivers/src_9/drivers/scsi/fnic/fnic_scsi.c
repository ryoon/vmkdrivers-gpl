				   ...
				   struct scsi_cmnd *sc)
{
	if (io_req->sgl_list_pa)
		pci_unmap_single(fnic->pdev, io_req->sgl_list_pa,
				 ...
				 + SCSI_SENSE_BUFFERSIZE, PCI_DMA_TODEVICE);
	scsi_dma_unmap(sc);

	if (io_req->sgl_list_alloc) {
					  ...
					  u32 sg_count)
{
	...
	/* We need the sense buffer irrespective of contiguous or
	 * non-contiguous sg elements.
	 */
...
int fnic_queuecommand(struct scsi_cmnd *sc, void (*done)(struct scsi_cmnd *))
{
	...
	if (!io_req) {
		ret = SCSI_MLQUEUE_HOST_BUSY;
		FNIC_SCSI_DBG(KERN_DEBUG, fnic->lport->host,
			"Failed to allocate io_req from fnic io_req_pool!\n");
	...
	/* We always have to allocate the sense buffer, so
	 * always allocate memory for sgl list irrespective of sgl_count
	 */
	...
	if (!io_req->sgl_list) {
		FNIC_SCSI_DBG(KERN_DEBUG, fnic->lport->host,
			"Failed to allocate sgl_list for sgl_type %d "
			"from io_sgl_pool!\n",io_req->sgl_type);
		ret = SCSI_MLQUEUE_HOST_BUSY;
		scsi_dma_unmap(sc);
		scsi_host_end_tag(sc->device->host, sc);
...
static inline void fnic_copy_sense(struct scsi_cmnd *cmd,
					 ...
					 struct fcpio_fw_req *desc)
{
	...
	case FCPIO_SUCCESS:
		...
		/* Copy sense buffer */
		fnic_copy_sense(sc, io_req, icmnd_cmpl->sense_len);
...
#if defined(__VMKLNX__)
		sc->result = (DID_RESET << 16) | icmnd_cmpl->scsi_status;
#else
				       ...
				       struct fnic_io_req *io_req)
{
	...
	fnic_queue_wq_copy_desc_itmf(wq, scsi_cmd_get_tag(sc) |
				     FNIC_TAG_DEV_RST,
				     ...
				     struct scsi_cmnd *lr_sc)
{
	...
	for (tag = 0; tag < FNIC_MAX_IO_REQ; tag++) {
...
#if defined(__VMKLNX__)
                /*
                 * Any pending IO issued prior to reset might get caught by
                 * BUG_ON. Indeed, we need to set FNIC_IOREQ_ABTS_PENDING to
                 * indicate the IO is abort pending. When IO is completed, the
                 * IO will be handed over and handled in this function.
                 */
		CMD_STATE(sc) = FNIC_IOREQ_ABTS_PENDING;
#else
		...
		fnic_release_ioreq_buf(fnic, io_req, sc);
                 ...
                 */
                if (sc->scsi_done) {
                    sc->result = DID_RESET << 16; // let upper layer to retry
                    sc->scsi_done(sc);
                 }
#endif
		mempool_free(io_req, fnic->io_req_pool);
