...
#if defined(__VMKLNX__)
        int_to_scsilun_with_sec_lun_id(sc->device->lun, &fc_lun,
...
fnic_abort_cmd_end:
	FNIC_TRACE((u64)fnic_abort_cmd, sc->device->host->host_no, tag,
		  (u64)sc, jiffies_to_msecs(jiffies - start_time),
		  0, ((u64)sc->cmnd[0] << 32 |
		  (u64)sc->cmnd[2] << 24 | (u64)sc->cmnd[3] << 16 |
		  (u64)sc->cmnd[4] << 8 | sc->cmnd[5]),
		  (((u64)CMD_FLAGS(sc) << 32) | CMD_STATE(sc)));
...
fnic_device_reset_end:
	FNIC_TRACE((u64)fnic_device_reset, sc->device->host->host_no, tag,
		  (u64)sc, jiffies_to_msecs(jiffies - start_time),
		  0, ((u64)sc->cmnd[0] << 32 |
		  (u64)sc->cmnd[2] << 24 | (u64)sc->cmnd[3] << 16 |
		  (u64)sc->cmnd[4] << 8 | sc->cmnd[5]),
