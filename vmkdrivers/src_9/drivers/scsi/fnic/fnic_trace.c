 ...
 * $Id: fnic_trace.c 95328 2012-02-09 01:42:58Z hiralpat $
 */
	...
	if (wr_idx < rd_idx) {
		while (1) {
			...
			len += snprintf(fnic_dbgfs_prt->buffer + len,
				  ...
				  "%16lu.%16lu %16llx %8x %8x %16llx %16llx "
				  "%16llx %16llx %16llx\n", val.tv_sec,
				  val.tv_nsec, tbp->fnaddr, tbp->host_no,
				  tbp->tag, tbp->data[0], tbp->data[1],
				  tbp->data[2], tbp->data[3], tbp->data[4]);
			rd_idx++;
			/*
...
int fnic_get_stats_data(struct stats_debug_info *debug, struct fnic_stats *stats)
{
	...
	len += snprintf(debug->debug_buffer + len, buf_size - len,
		  ...
		  (u64)atomic64_read(&stats->io_stats.tag_alloc_failures),
	...
	len += snprintf(debug->debug_buffer + len, buf_size - len,
		  ...
		  (u64)atomic64_read(&stats->reset_stats.device_reset_terminates),
		  ...
		  (u64)atomic64_read(&stats->reset_stats.fnic_reset_completions),
	...
	len += snprintf(debug->debug_buffer + len, buf_size - len,
		  ...
		  (u64)stats->misc_stats.last_isr_time, val1.tv_sec, val1.tv_nsec,
		  (u64)stats->misc_stats.last_ack_time, val2.tv_sec, val2.tv_nsec,
		  ...
		  (u64)atomic64_read(&stats->misc_stats.abts_cpwq_alloc_failures),
		  (u64)atomic64_read(&stats->misc_stats.devrst_cpwq_alloc_failures),
