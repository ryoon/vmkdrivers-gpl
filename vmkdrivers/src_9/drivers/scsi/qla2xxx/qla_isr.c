...
skip_rio:
	...
	case MBA_LOOP_DOWN:		/* Loop Down Event */
		...
		DEBUG2(printk("scsi(%ld): Asynchronous LOOP DOWN "
			"mbx1=%xh mbx2=%xh mbx3=%xh mbx7=%xh.\n", vha->host_no, 
			mb[1], mb[2], mb[3], mbx));
		...
		if (scsi_status & (SS_RESIDUAL_UNDER | SS_RESIDUAL_OVER)) {
			...
			if (!lscsi_status &&
				...
				qla_printk(KERN_INFO, ha,
					   ...
					   "detected cmd:0x%x (%d of %d bytes)\n",
					   vha->host_no, cp->device->channel, cp->device->id,
					   cp->device->lun, cp->cmnd[0], resid, scsi_bufflen(cp));
	...
	case CS_DATA_UNDERRUN:
		...
		if (scsi_status & SS_RESIDUAL_UNDER) {
			CMD_RESID_LEN(cp) = resid;
 			...
 			if (IS_FWI2_CAPABLE(ha) && fw_resid_len != resid_len) {
 				DEBUG2(printk("scsi(%ld:%d:%d:%d) Dropped "
 			    	    "frame(s) 0x%x-0x%x (%d of %d bytes) resid mismatch "
				    "resid=0x%x fw_resid=0x%x cdb=0x%x ox_id=0x%x\n",
 				    vha->host_no, cp->device->channel, cp->device->id,
					cp->device->lun, comp_status, scsi_status, resid,
					scsi_bufflen(cp), resid_len, fw_resid_len,
					cp->cmnd[0], le16_to_cpu(sts24->ox_id)));
 			...
 			if (!lscsi_status &&
				 ...
				 */
 				qla_printk(KERN_INFO, ha,
 					   "scsi(%ld:%d:%d:%d): Mid-layer underflow 0x%x-0x%x"
					   "detected (%d of %d bytes) resid=0x%x fw_resid=0x%x "
					   "cdb=0x%x ox_id=0x%x\n",
						vha->host_no, cp->device->channel, cp->device->id,
						cp->device->lun, comp_status, scsi_status, resid,
						scsi_bufflen(cp), resid_len, fw_resid_len, cp->cmnd[0],
						le16_to_cpu(sts24->ox_id));
			...
			/* Note: ox_id will be garbage for non IS_FWI2_CAPABLE(ha) hbas */
 			DEBUG2(printk("scsi(%ld:%d:%d:%d) 0x%x-0x%x Dropped "
			    "frame(s) detected (%d of %d bytes) resid=0x%x "
			    "fw_resid=0x%x cdb=0x%x ox_id=0x%x\n",
 			    vha->host_no, cp->device->channel, cp->device->id,
 			    cp->device->lun, comp_status, scsi_status, resid,
			    scsi_bufflen(cp), resid_len, fw_resid_len, cp->cmnd[0],
			    le16_to_cpu(sts24->ox_id)));
...
#define MIN_MSIX_COUNT	2
	...
	printk("%s: max_rsp_queues = %d, max_req_queues = %d\n", __func__,
...
skip_msi:
	...
	ret = request_irq(ha->pdev->irq, ha->isp_ops->intr_handler,
 	    ha->flags.msi_enabled ? 
	    IRQF_DISABLED : IRQF_DISABLED|IRQF_SHARED, 
