...
#define NPIV_CONFIG_SIZE	(16*1024)
	...
	for (i = 0; cnt; cnt--, entry++, i++) {
		...
		if(!vha->flags.init_done && i < QLA_PRECONFIG_VPORTS) {
			...
			if (fvha == NULL) {
				/* GSS: Something went wrong. Cleanup. */
...
qla2xxx_npiv_cleanup(scsi_qla_host_t *vha)
{
	...
	list_for_each_entry_safe(vp, tvp, &ha->vp_list, list) {
		...
		/* GSS: Needs additional logic to handle this. As no DPC,
...
int qla2xxx_read_fcp_prio_cfg(scsi_qla_host_t *vha)
{
	...
	ha->isp_ops->read_optrom(vha, (uint8_t *)vha->fcp_prio_cfg,
	    fcp_prio_addr << 2, FCP_PRIO_CFG_HDR_SIZE);

	if (!qla2xxx_fcp_prio_cfg_valid(vha->fcp_prio_cfg, 0))
		goto fail;

	/* read remaining FCP CMD config data from flash */
	fcp_prio_addr += (FCP_PRIO_CFG_HDR_SIZE >> 2);
	len = vha->fcp_prio_cfg->num_entries * FCP_PRIO_CFG_ENTRY_SIZE;
	max_len = FCP_PRIO_CFG_SIZE - FCP_PRIO_CFG_HDR_SIZE;

	ha->isp_ops->read_optrom(vha, (uint8_t *)&vha->fcp_prio_cfg->entry[0],
