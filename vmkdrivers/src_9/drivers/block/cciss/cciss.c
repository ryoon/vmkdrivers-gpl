...
#define DRIVER_NAME "HP CISS Driver (v 3.6.14-10vmw)"
#define DRIVER_VERSION CCISS_DRIVER_VERSION(3,6,14)

/* Embedded module documentation macros - see modules.h */
MODULE_AUTHOR("Hewlett-Packard Company");
MODULE_DESCRIPTION("Driver for HP Controller SA5xxx SA6xxx version 3.6.14-10vmw");
MODULE_SUPPORTED_DEVICE("HP SA5i SA5i+ SA532 SA5300 SA5312 SA641 SA642 SA6400"
			" SA6i P600 P400 E200 E200i P800 E500");
MODULE_VERSION("3.6.14-10vmw");
MODULE_LICENSE("GPL");
...
#if defined(__VMKLNX__)
	seg = vmklnx_blk_rq_map_sg(creq, &sglist);
#else /* !defined(__VMKLNX__) */
...
#if defined(__VMKLNX__)
	for_each_sg(sglist, sg, seg, i) {
		c->SG[i].Len = sg_dma_len(sg);
		temp64.val = sg_dma_address(sg);
#else /* !defined(__VMKLNX__) */
	for (i = 0; i < seg; i++) {
				    ...
				    const struct pci_device_id *ent)
{
	...
	vmklnx_block_register_poll_handler(hba[i]->major, 
					   hba[i]->intr[SIMPLE_MODE_INT],
					   do_cciss_intr,
					   hba[i]);
#endif /*  defined(__VMKLNX__)  */
...
static void cciss_shutdown(struct pci_dev *pdev)
{
	...
	printk(KERN_INFO "cciss_shutdown: flushing cache on controller\n");
