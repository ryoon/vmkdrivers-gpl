		      ...
		      struct scsi_cmnd* srb)
{
	...
	/* XXX PR 688894 - Check if we are doing a dump */
	if (unlikely(scsi_dump_active(us->srb)))
	{
		if (scsi_sg_count(srb) != 1) 
   			_VMKLNX_USB_STOR_WARN("Error: more than 1 element in the coredump s/g list: %d\n", us->srb, scsi_sg_count(srb)); 
		VMK_ASSERT(scsi_sg_count(srb) == 1);
		vmk_sgarray *sga = ((struct scatterlist *)srb->request_buffer)->vmksga;
		VMK_ASSERT(sga);
		srb->request_bufferMA = (unsigned long) sga->elem[0].addr;
		srb->request_buffer = phys_to_virt(srb->request_bufferMA);
	}

	/* are we scatter-gathering? */
	if (srb->use_sg && likely(!scsi_dump_active(us->srb))) {
