			       ...
			       int level, int index, int count)
{
	...
	if (level > MAX_TOPO_LEVEL)
		return 0;
#if defined(__VMKLNX__)
	if (!(pages_start = kzalloc(2 * PAGE_SIZE, GFP_KERNEL)))
                return -ENOMEM;
#else
...
#if defined(__VMKLNX__)
	if (usbdev->parent && vmklnx_usb_devnum(usbdev->parent) != -1)
		parent_devnum = vmklnx_usb_devnum(usbdev->parent);
#else
	...
	data_end = pages_start + sprintf(pages_start, format_topo,
			...
			index, count, vmklnx_usb_devnum(usbdev),
			speed, usbdev->maxchild);
	// Display USB passthrough availability information
	data_end += sprintf(data_end, format_passthrough,
			usbdev->passthrough ? "Available" : "Not available",
			!usbdev->passthrough ? "" :
				(usbdev->in_use ? ", currently in use" : ""));
#else
	data_end = pages_start + sprintf(pages_start, format_topo,
			bus->busnum, level, parent_devnum,
...
#define USBDEVICES "usbdevices"

static int usbdevices_major = 0;

int usbfs_add_devices(void)
{
	if (usbdevices_major) {
		return 0;
	}

	usbdevices_major = register_chrdev(0, USBDEVICES, &usbfs_devices_fops);
	if (usbdevices_major < 0) {
		printk ("usbfs: failed to register %s (%d)\n",
			USBDEVICES, usbdevices_major);
		usbdevices_major = 0;
	}

	return usbdevices_major ? 0 : 1;
}

void usbfs_remove_devices(void)
{
	if (!usbdevices_major) {
		return;
	}

	unregister_chrdev(usbdevices_major, USBDEVICES);
}
