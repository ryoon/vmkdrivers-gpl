...
#ifdef __VMKLNX__
static int rtl8169_rx_interrupt(struct net_device *, struct rtl8169_private *, void __iomem *, u32 budget, u8 in_netpoll);
#else
...
void rtl8169_rar_set(struct rtl8169_private *tp, uint8_t *addr, uint32_t index);

static void rtl8169_phy_power_up (struct net_device *dev);
static void rtl8169_phy_power_down (struct net_device *dev);

static void rtl8169_trx_reset(void __iomem *ioaddr);
...
static unsigned int rtl8169_xmii_link_ok(struct net_device *dev)
{
	...
	spin_lock_irqsave(&tp->phy_lock, flags);
	mdio_write(ioaddr, 0x1f, 0x0000);
	retval = mdio_read(ioaddr, MII_BMSR) & BMSR_LSTATUS;
	spin_unlock_irqrestore(&tp->phy_lock, flags);
...
static void rtl8169_xmii_reset_enable(struct net_device *dev)
{
	...
	for(i = 0; i < 2500; i++) {
		spin_lock_irqsave(&tp->phy_lock, flags);
		mdio_write(ioaddr, 0x1f, 0x0000);
		status = mdio_read(ioaddr, MII_BMSR) & BMCR_RESET;
		spin_unlock_irqrestore(&tp->phy_lock, flags);
				...
				struct ethtool_drvinfo *info)
{
	...
	strncpy(info->fw_version, firmware_version, 32);
}
...
static void rtl8169_hw_phy_config(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_6) {
		...
		mdio_write(ioaddr, 0x04, 0x0000);
		mdio_write(ioaddr, 0x03, 0x00a1);
		mdio_write(ioaddr, 0x02, 0x0008);
		mdio_write(ioaddr, 0x01, 0x0120);
		mdio_write(ioaddr, 0x00, 0x1000);
		mdio_write(ioaddr, 0x04, 0x0800);
		...
		mdio_write(ioaddr, 0x04, 0xa000);
		mdio_write(ioaddr, 0x03, 0xdf01);
		mdio_write(ioaddr, 0x02, 0xdf20);
		mdio_write(ioaddr, 0x01, 0xff95);
		mdio_write(ioaddr, 0x00, 0xba00);
		mdio_write(ioaddr, 0x04, 0xa800);
		mdio_write(ioaddr, 0x04, 0xf000);
		mdio_write(ioaddr, 0x03, 0xdf01);
		mdio_write(ioaddr, 0x02, 0xdf20);
		mdio_write(ioaddr, 0x01, 0x101a);
		mdio_write(ioaddr, 0x00, 0xa0ff);
		mdio_write(ioaddr, 0x04, 0xf800);
		mdio_write(ioaddr, 0x04, 0x0000);
		mdio_write(ioaddr, 0x1f, 0x0000);

		mdio_write(ioaddr, 0x1f, 0x0001);
		mdio_write(ioaddr, 0x0b, 0x8480);
		mdio_write(ioaddr, 0x1f, 0x0000);

		mdio_write(ioaddr, 0x1f, 0x0001);
		mdio_write(ioaddr, 0x18, 0x67c7);
		mdio_write(ioaddr, 0x04, 0x2000);
		mdio_write(ioaddr, 0x03, 0x002f);
		mdio_write(ioaddr, 0x02, 0x4360);
		mdio_write(ioaddr, 0x01, 0x0109);
		mdio_write(ioaddr, 0x00, 0x3022);
		mdio_write(ioaddr, 0x04, 0x2800);
		mdio_write(ioaddr, 0x1f, 0x0000);
		...
		mdio_write(ioaddr, 0x14, 0xfb54);
		mdio_write(ioaddr, 0x18, 0xf5c7);
		mdio_write(ioaddr, 0x1f, 0x0000);
		...
		mdio_write(ioaddr, 0x1f, 0x0001);
		mdio_write(ioaddr, 0x06, 0x006e);
		mdio_write(ioaddr, 0x08, 0x0708);
		mdio_write(ioaddr, 0x15, 0x4000);
		mdio_write(ioaddr, 0x18, 0x65c7);
		...
		mdio_write(ioaddr, 0x03, 0xff41);
		mdio_write(ioaddr, 0x02, 0xdf60);
		mdio_write(ioaddr, 0x01, 0x0140);
		mdio_write(ioaddr, 0x00, 0x0077);
		mdio_write(ioaddr, 0x04, 0x7800);
		mdio_write(ioaddr, 0x04, 0x7000);

		mdio_write(ioaddr, 0x03, 0x802f);
		mdio_write(ioaddr, 0x02, 0x4f02);
		mdio_write(ioaddr, 0x01, 0x0409);
		mdio_write(ioaddr, 0x00, 0xf0f9);
		mdio_write(ioaddr, 0x04, 0x9800);
		mdio_write(ioaddr, 0x04, 0x9000);
		...
		mdio_write(ioaddr, 0x02, 0xdf20);
		mdio_write(ioaddr, 0x01, 0x0140);
		mdio_write(ioaddr, 0x00, 0x00bb);
		mdio_write(ioaddr, 0x04, 0xb800);
		mdio_write(ioaddr, 0x04, 0xb000);

		mdio_write(ioaddr, 0x03, 0xdf41);
		mdio_write(ioaddr, 0x02, 0xdc60);
		mdio_write(ioaddr, 0x01, 0x6340);
		mdio_write(ioaddr, 0x00, 0x007d);
		mdio_write(ioaddr, 0x04, 0xd800);
		mdio_write(ioaddr, 0x04, 0xd000);

		mdio_write(ioaddr, 0x03, 0xdf01);
		mdio_write(ioaddr, 0x02, 0xdf20);
		mdio_write(ioaddr, 0x01, 0x100a);
		mdio_write(ioaddr, 0x00, 0xa0ff);
		mdio_write(ioaddr, 0x04, 0xf800);
		mdio_write(ioaddr, 0x04, 0xf000);

		mdio_write(ioaddr, 0x1f, 0x0000);
		mdio_write(ioaddr, 0x0b, 0x0000);
		mdio_write(ioaddr, 0x00, 0x9200);
	}
	spin_unlock_irqrestore(&tp->phy_lock, flags);
}
...
static void
rtl8169_phy_power_up (struct net_device *dev)
{
	struct rtl8169_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
...
static void
rtl8169_phy_power_down (struct net_device *dev)
{
	struct rtl8169_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
...
static int rtl8169_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	if (new_mtu < ETH_ZLEN || new_mtu > Jumbo_Frame_7k)
		return -EINVAL;
...
#ifdef __VMKLNX__
	rtl8169_rx_interrupt(dev, tp, tp->mmio_addr, ~(u32)0, 0);
#else
...
rtl8169_rx_interrupt(struct net_device *dev,
		     struct rtl8169_private *tp,
#ifdef __VMKLNX__
		     void __iomem *ioaddr, u32 budget, u8 in_netpoll)
#else
...
#ifdef __VMKLNX__
	work_done = rtl8169_rx_interrupt(dev, tp, ioaddr, (u32) budget, 1);
#else
