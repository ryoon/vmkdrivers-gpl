...
#ifndef __VMKLNX__
	/* net scheduler of ESXi need TX coalesce when QP is enabled */
...
static bool is_cim_supported(struct igb_adapter *adapter)
{
...
static bool validate_smbios_entry(const struct smbios_table *tbl,
				  struct igb_adapter *adapter)
{
	u32 size = tbl->EntryPointLength;
	u32 offset;
	u8 *byte;
	u8 csum;
...
find_smbios_entry(struct smbios_table *stbl, struct igb_adapter *adapter)
{
	void *vbase;
	u32 offset;
	u32 size = SM_ADDR_HIGH - SM_ADDR_LOW + 1;
	struct smbios_table *tbl = 0;
	...
	for (offset = 0; offset < size; offset += 0x10) {
		tbl = (struct smbios_table *)(vbase + offset);
...
static bool get_smbios_tables(void *smbios, struct igb_adapter *adapter)
{
	void *vbase;
	if (!smbios)
		return false;
...
static int get_pcie_error_info(u32 *pcie_errors, u32 num_regs, struct igb_adapter *adapter)
{
        int cap;
        int res;
        u32 ix;
        ...
        for (ix = 0; ix < num_regs; ++ix) {
                res = pci_read_config_dword(adapter->pdev, cap + PCI_ERR_HEADER_LOG +
                                                ix * sizeof(u32), &pcie_errors[ix]);
                if (res < 0)
                        return res;;
        }
        return num_regs;
...
static int get_link_status(u16 *link_status, struct igb_adapter *adapter)
{
        int cap;
        int res;
        ...
        res = pci_read_config_word(adapter->pdev, cap + PCI_EXP_LNKSTA, link_status);
        if (res < 0) {
...
/**
 * igb_intelcim_ioctl
 * @netdev: Pointer to netdev device
 * @ifr: user space request data
 *
 * called by CIM IOCTL to run diagnostics.
 * Basically, this function is a wrapper for ethtool functions.
 **/
int igb_intelcim_ioctl(struct net_device *netdev, struct ifreq *ifr)
{
	void *useraddr = (void *)ifr->ifr_data;
	struct igb_intelcim_ioctl_req req;
	struct igb_adapter *adapter = netdev_priv(netdev);
	int ret = 0;
	...
	case INTELCIM_ENUMDIAGS:
		{
			...
			gstrings.len = igb_diag_test_count(netdev);
			...
			igb_get_strings(netdev, gstrings.string_set, data);

			if (copy_to_user(useraddr, &gstrings,
					sizeof(gstrings))) {
				DPRINTK(DRV, ERR, "Cannot copy string results to user space\n");
				kfree(data);
				return -EFAULT;
			}
	...
	case INTELCIM_RUNDIAG:
		{
			...
			test.len = igb_diag_test_count(netdev);
			...
			igb_diag_test(netdev, &test, data);

			if (copy_to_user(useraddr, &test, sizeof(test))) {
				DPRINTK(DRV, ERR, "Cannot copy test results to user space\n");
				kfree(data);
				return -EFAULT;
			}
	...
	case INTELCIM_FNDSMB:
		{
			...
			if (!find_smbios_entry(&smbtbl, adapter)) {
				...
				if (copy_to_user(useraddr,
					...
					DPRINTK(DRV, ERR, "Cannot copy to user memory\n");
					return -EFAULT;
				}
			}
		}
	...
	case INTELCIM_WRITEMEM:
		{
			...
			if (copy_from_user(data, useraddr, buf.len * sizeof(u8))) {
				DPRINTK(DRV, ERR, "Cannot copy memory from user space\n");
				kfree(data);
				return -EFAULT;
			}
			...
			if (!vbase) {
				DPRINTK(DRV, ERR, "Failed to map buffer memory\n");
				return -EFAULT;
			}
	...
	case INTELCIM_READMEM:
		{
			...
			useraddr += sizeof(buf.addr) + sizeof(buf.len);
			if (copy_to_user(useraddr, data, buf.len * sizeof(u8))) {
				DPRINTK(DRV, ERR, "Cannot copy mem contents to user space\n");
				kfree(data);
				return -EFAULT;
			}
        ...
        case INTELCIM_GET_PCIE_ERROR_INFO:
                {
                        ...
                        DPRINTK(DRV, DEBUG, "INTELCIM_GET_PCIE_ERROR_INFO ioctl called\n");
			...
			if (!is_cim_supported(adapter))
				return -EPERM;
                        ...
                        pcie_errors = kmalloc(info.num_regs * sizeof(u32), GFP_USER);
                        if (!pcie_errors) {
                                DPRINTK(DRV, ERR, "Cannot allocate memory\n");
                                return -ENOMEM;
                        }
                        ...
                        if((get_pcie_error_info((u32*)pcie_errors,
                                                (u32)info.num_regs, adapter)) < 0) {
                                DPRINTK(DRV, ERR, "Cannot get pcie error info\n");
                                kfree(pcie_errors);
                                return -EFAULT;
                        }

                        useraddr += sizeof(info.num_regs);
                        if (copy_to_user(useraddr, pcie_errors, info.num_regs * sizeof(u32))) {
                                DPRINTK(DRV, ERR, "Cannot copy mem contents to user space\n");
                                kfree(pcie_errors);
                                return -EFAULT;
                        }
        ...
        case INTELCIM_GET_PCI_LINK_STATUS:
                {
                        ...
                        DPRINTK(DRV, DEBUG, "INTELCIM_GET_PCI_LINK_STATUS ioctl called\n");
			...
			if (!is_cim_supported(adapter))
				return -EPERM;
                        ...
                        if ((get_link_status(&link_status, adapter)) < 0) {
                                DPRINTK(DRV, ERR, "Unable to get link status\n");
                                return -EFAULT;
                        } else {
                                if (copy_to_user(useraddr,
