			      ...
			      int i)
{
	...
	/* Allocate new page for every fragment */
	...
	for (buf_ind = 0; buf_ind < priv->prof->rx_ring_size; buf_ind++) {
		for (ring_ind = 0; ring_ind < priv->rx_ring_num; ring_ind++) {
			...
			if (priv->use_rx_frags)
				...
				err = mlx4_en_prepare_rx_desc_skb(priv, ring, ring->actual_size);
	...
	for (ring_ind = 0; ring_ind < priv->rx_ring_num; ring_ind++) {
		...
		while (ring->actual_size > new_size) {
			...
			if (priv->use_rx_frags)
				...
				mlx4_en_free_rx_desc_skb(priv, ring, ring->actual_size);
#endif  /* __VMKERNEL_MODULE__ */
		}
	}
				...
				struct mlx4_en_rx_ring *ring)
{
	...
	while (ring->cons != ring->prod) {
		...
		if (priv->use_rx_frags)
			...
			mlx4_en_free_rx_desc_skb(priv, ring, index);
#endif	/* __VMKERNEL_MODULE__ */
		++ring->cons;
	}
}
...
#else
	int stride = priv->use_rx_frags ?
			roundup_pow_of_two(sizeof(struct mlx4_en_rx_desc) +
			DS_SIZE * priv->num_frags) : 0;
#endif	/* __VMKERNEL_MODULE__ */
...
int mlx4_en_process_rx_cq(struct net_device *dev, struct mlx4_en_cq *cq, int budget)
{
	...
	struct sk_buff *skb;
	int index;
#ifdef __VMKERNEL_NETDEV_GRO_FEATURE_SUPPORT_
	int nr;
#endif	/* __VMKERNEL_NETDEV_GRO_FEATURE_SUPPORT_ */
	unsigned int length;
	int polled = 0;
	int ip_summed;
	struct ethhdr *ethh;
