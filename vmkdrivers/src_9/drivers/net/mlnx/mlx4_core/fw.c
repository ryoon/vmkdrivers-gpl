...
#define QUERY_FUNC_CAP_BASE_QPN_OFFSET		0X40
...
#define QUERY_FUNC_CAP_PHY_WQE_GID_OFFSET	0x8
	...
	if (vhcr->op_modifier == 1) {
		...
		MLX4_PUT(outbox->buf, field, QUERY_FUNC_CAP_PHY_WQE_GID_OFFSET);
	} else if (vhcr->op_modifier == 0) {
		field = 3 << 6; /* enable only ethernet and rdma */
		...
		size = dev->caps.base_sqpn;
		MLX4_PUT(outbox->buf, size, QUERY_FUNC_CAP_BASE_QPN_OFFSET);

		size = dev->caps.base_sqpn + 8 * (1 + MLX4_MFUNC_MAX + slave);
		MLX4_PUT(outbox->buf, size, QUERY_FUNC_CAP_BASE_TUNNEL_QPN_OFFSET);

		size = dev->caps.base_sqpn + 8 * (slave + 1);
...
int mlx4_QUERY_FUNC_CAP(struct mlx4_dev *dev, struct mlx4_func_cap *func_cap)
{
	...
	MLX4_GET(size, outbox, QUERY_FUNC_CAP_BASE_QPN_OFFSET);
	func_cap->base_qpn = size & 0xFFFFFF;
	...
	for (i = 1; i <= func_cap->num_ports; ++i) {
		...
		if (dev->caps.port_type[i] == MLX4_PORT_TYPE_IB) {
			MLX4_GET(field, outbox,
				 QUERY_FUNC_CAP_PHY_WQE_GID_OFFSET);
			if (field & (1 << 7)) {
...
#define MLX4_VF_PORT_NO_LINK_SENSE_MASK	0xE4
#define QUERY_PORT_CUR_MAX_GID_OFFSET		0x0c
#define QUERY_PORT_CUR_MAX_PKEY_OFFSET		0x0e
	...
	if (!err && dev->caps.function != slave) {
		if (dev->caps.port_type[vhcr->in_modifier]
			== MLX4_PORT_TYPE_ETH) {
