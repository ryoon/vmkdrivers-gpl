...
int mlx4_init_qp_table(struct mlx4_dev *dev)
{
	...
	/*
	 * reserve 8 (unused) tunnel QPs, 8 real SQPs, and, per function,
	 * 8 proxy SQPs (used by slaves), and 8 corresponding tunnel QPs (for master).
	 * Each proxy SQP works opposite its own Dom0 tunnel QP.
	 */
	...
	/*In mfunc the start of the sqp is diffrent:*/
	if (mlx4_is_mfunc(dev)) {
		dev->caps.sqp_start = dev->caps.base_sqpn + 8 * (dev->caps.function + 1);
