/*
 * Copyright (c) 2007, 2011-2013 Mellanox Technologies. All rights reserved.
 *
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
EXPORT_SYMBOL_GPL(mlx4_uc_steer_add);
#endif	/* NET QUEUE */
...
#if defined(__VMKLNX__) && defined(__VMKNETDDI_QUEUEOPS__)
EXPORT_SYMBOL_GPL(mlx4_uc_steer_release);
#endif	/* NET QUEUE */
				...
				u8 op_mod, struct mlx4_cmd_mailbox *inbox)
{
	...
	if ((slave != dev->caps.function) && reset_qkey_viols)
		return -EINVAL;
	...
	/* Only clear mailbox for guests.  Guests may ONLY set their own CAP
	*  Mask. Master may be setting MTU or PKEY table size, or VL_cap
	*/
	if (slave != dev->caps.function)
...
int mlx4_SET_PORT(struct mlx4_dev *dev, u8 port, int pkey_tbl_size)
{
	...
	for (vl_cap = 8; vl_cap >= 1; vl_cap >>= 1) {
		...
		if (pkey_tbl_size >= 0 && mlx4_is_master(dev)) {
			((__be32 *) mailbox->buf)[0] |=
				cpu_to_be32(1 << MLX4_CHANGE_PORT_PKEY_TBL_SZ);
