...
/*
 * When working with netq the default queue is promisc and we don't
 * want it to continue receiving the packets from other queues as well
 */
...
/*
 * Add new entry to steering data structure.
 * Not need All promisc QPs should be added as well.
 */
static int new_steering_entry(struct mlx4_dev *dev, u8 port,
			      enum mlx4_steer_type steer,
			      unsigned int index, u32 qpn)
