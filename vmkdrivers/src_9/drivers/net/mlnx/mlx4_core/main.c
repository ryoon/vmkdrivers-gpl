...
#else
module_param_named(max_vfs, num_vfs, int, 0444);
MODULE_PARM_DESC(max_vfs, "enable #max_vfs functions if max_vfs > 0");
#endif  /* __VMKERNEL_MODULE__ */
...
module_param(mtu_4k, int, 0644);
MODULE_PARM_DESC(mtu_4k, " configure 4k mtu (mtu_4k > 0)");
#endif	/* NOT __VMKERNEL_MLX4_EN_SUPPORT__ */
...
#else
#ifdef __VMKERNEL_MLX4_EN_SUPPORT__
					dev->caps.port_type[i] = MLX4_PORT_TYPE_ETH;
#else
					dev->caps.port_type[i] = MLX4_PORT_TYPE_IB;
#endif  /* __VMKERNEL_MLX4_EN_SUPPORT__ */
#endif	/* __VMKERNEL_MLNX_SENSNE_ENABLE__ */
				else
...
int mlx4_get_parav_qkey(struct mlx4_dev *dev, u32 qpn, u32 *qkey)
{
	...
	if (qpn >= dev->caps.base_sqpn + 8 + 16 * MLX4_MFUNC_MAX ||
	    qpn < dev->caps.base_sqpn + 8)
		return -EINVAL;

	if (qpn >= dev->caps.base_sqpn + 8 * (MLX4_MFUNC_MAX + 1))
		/* tunnel qp */
		qk += qpn - (dev->caps.base_sqpn + 8 * (MLX4_MFUNC_MAX + 1));
	else
		qk += qpn - (dev->caps.base_sqpn + 8);
	*qkey = qk;
	return 0;
}
...
static int mlx4_slave_cap(struct mlx4_dev *dev)
{
	...
	dev->caps.base_sqpn = func_cap.base_qpn;
...
#ifdef __VMKERNEL_SYSFS_SUPPORT__
...
#endif	/* __VMKERNEL_SYSFS_SUPPORT__ */

#ifndef __mtrack_h_ /* memtrack causes conflict with ib_verbs */
enum ibta_mtu {
	IB_MTU_256  = 1,
	IB_MTU_512  = 2,
	IB_MTU_1024 = 3,
	...
	if (!mlx4_is_slave(dev)) {
		...
		for (port = 1; port <= dev->caps.num_ports; port++) {
			...
			if (dev->caps.port_type[port] == MLX4_PORT_TYPE_IB)
				mlx4_info(dev, "port=%u IB MTU=%u\n",
					  port, ibta_mtu_to_int(dev->caps.port_ib_mtu[port]));
				...
				for (i = 0; i < dev->num_slaves; i++)
					if (i != dev->caps.function)
						priv->mfunc.master.slave_state[i].ib_cap_mask[port] =
							ib_port_default_caps;
				pkey_tbl_size = dev->caps.pkey_table_len[port];
			}
...
#ifdef __VMKERNEL_LARGE_DMA_SEG_ENABLE__
	/* Allow large DMA segments, up to the firmware limit of 1 GB */
	dma_set_max_seg_size(&pdev->dev, 1024 * 1024 * 1024);
#endif	/* __VMKERNEL_LARGE_DMA_SEG_ENABLE__ */
...
#ifdef __VMKERNEL_MODULE__
			if (!vmklnx_enable_vfs(pdev, num_vfs, NULL, NULL)) {
#else
...
#ifdef __VMKERNEL_MODULE__
		vmklnx_disable_vfs(pdev, num_vfs, NULL, NULL);
#else
...
static void mlx4_remove_one(struct pci_dev *pdev)
{
	...
	if (dev) {
		...
		if (mlx4_is_master(dev))
			mlx4_free_resource_tracker(dev, 1);
...
static DEFINE_PCI_DEVICE_TABLE(mlx4_pci_table) = {
	...
	{ PCI_VDEVICE(MELLANOX, 0x1006), MLX4_VF },	/* MT27511 Family */
	{ PCI_VDEVICE(MELLANOX, 0x1007), 0 },		/* MT27520 Family */
	{ PCI_VDEVICE(MELLANOX, 0x1008), MLX4_VF },	/* MT27521 Family */
	{ PCI_VDEVICE(MELLANOX, 0x1009), 0 },		/* MT27530 Family */
	{ PCI_VDEVICE(MELLANOX, 0x100a), MLX4_VF },	/* MT27531 Family */
	{ PCI_VDEVICE(MELLANOX, 0x100b), 0 },		/* MT27540 Family */
	{ PCI_VDEVICE(MELLANOX, 0x100c), MLX4_VF },	/* MT27541 Family */
	{ PCI_VDEVICE(MELLANOX, 0x100d), 0 },		/* MT27550 Family */
	{ PCI_VDEVICE(MELLANOX, 0x100e), MLX4_VF },	/* MT27551 Family */
	{ PCI_VDEVICE(MELLANOX, 0x100f), 0 },		/* MT27560 Family */
	{ PCI_VDEVICE(MELLANOX, 0x1010), MLX4_VF },	/* MT27561 Family */
	{ 0, }
};
...
#ifdef __MEMTRACK__
	init_memtrack_module("mlx4_core");
#endif
