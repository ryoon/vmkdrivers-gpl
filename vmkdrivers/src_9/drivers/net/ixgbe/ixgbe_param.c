...
void __devinit ixgbe_check_options(struct ixgbe_adapter *adapter)
{
	...
	}
	{
		...
		if (adapter->hw.mac.type != ixgbe_mac_82598EB) {
			...
			/* only enabled CNA when MSIX is enabled */   
			if (!(*aflags & IXGBE_FLAG_MSIX_CAPABLE))   
				cna_qs = 0;   
			if (cna_qs) {
				adapter->flags2 |= IXGBE_FLAG2_CNA_ENABLED;
				adapter->num_tx_cna_queues = cna_qs;
				adapter->num_rx_cna_queues = cna_qs;
		...
		if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED) {
			/* disable RSC and PS, they don't get along with CNA/FCoE */
...
#ifdef __VMKLNX__
                        /* for ESX, Oplin has PS disabled as well */
                        if (adapter->hw.mac.type == ixgbe_mac_82598EB)
                                 RxBufferMode[bd] = IXGBE_RXBUFMODE_1BUF_ALWAYS;
#endif
