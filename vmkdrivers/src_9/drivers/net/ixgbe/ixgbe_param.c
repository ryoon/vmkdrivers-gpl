...
void __devinit ixgbe_check_options(struct ixgbe_adapter *adapter)
{
	...
	{ /* Receive-Side Scaling (RSS) */
		...
		if (*aflags & IXGBE_FLAG_RSS_ENABLED) {
			...
			/* do not use Flow Director when RSS is enabled*/
...

	{
		...
		if (adapter->hw.mac.type != ixgbe_mac_82598EB) {
			...
			/* Update flag after parameter validation */
			if (cna_enabled_manually)
				cna_enabled_manually = cna_qs;
	...
	{ /* Virtual Machine Device Queues (VMDQ) */
		...
		/* Update flag if VMDQ disabled in interoperability check */
		if (vmdq_enabled_manually && 
		    !(*aflags & IXGBE_FLAG_VMDQ_ENABLED))
			vmdq_enabled_manually = false;
	...
	{ /* Single Root I/O Virtualization (SR-IOV) */
		...
		if (*aflags & IXGBE_FLAG_SRIOV_ENABLED) {
			/* Only allow CNA + SRIOV if CNA enabled manually */
			if (!cna_enabled_manually) {
				*aflags &= ~IXGBE_FLAG_DCB_CAPABLE;
				*aflags &= ~IXGBE_FLAG_DCB_ENABLED;
				adapter->flags2 &= ~IXGBE_FLAG2_CNA_ENABLED;
				...
				DPRINTK(PROBE, INFO,
				        "CNA turned off when SR-IOV is "
					"enabled.\n");
			}
			/* If VMDq not enabled manually, only allow VMDQ=2 */
			if (!vmdq_enabled_manually &&
			   (*aflags & IXGBE_FLAG_VMDQ_ENABLED)) {
				feature[RING_F_VMDQ].indices = 2;
				adapter->flags2 |= IXGBE_FLAG2_VMDQ_DEFAULT_OVERRIDE;
				DPRINTK(PROBE, INFO,
				        "VMDQ reduced to 2 when SR-IOV is "
					"enabled.\n");
			}
			if (!(*aflags & IXGBE_FLAG_SRIOV_CAPABLE)) {
				...
				*aflags &= ~IXGBE_FLAG_SRIOV_ENABLED;
				 ...
				 * it down to save room for VMDQ, RSS, CNA
				 * queues or at least one default queue.  If
				 * CNA or RSS are enabled, max queues is 32
				 * (one for default queue), rather than 64.
				 * CNA and RSS are not supported simultaneously.
				 */
				if (feature[RING_F_FCOE].indices && 
				    feature[RING_F_VMDQ].indices)
					max_vfs = 32 -
						feature[RING_F_FCOE].indices;
				else if (feature[RING_F_FCOE].indices)
					max_vfs = 31 -
						feature[RING_F_FCOE].indices;
				/* RSS uses one pool in 32 pool mode */
				if (feature[RING_F_RSS].indices == 
				    IXGBE_ESX_RSS_QUEUES && 
				    feature[RING_F_VMDQ].indices)
					max_vfs = 32 - 1;
				else if (feature[RING_F_RSS].indices == 
				    IXGBE_ESX_RSS_QUEUES)
				    	max_vfs = 31 - 1;
	...
	{ /* Flow Director filtering mode */
		...
		/*
		 * When loading ixgbe module with VMDQ disabled, as soon as
		 * heavy traffic starts, there will be a large number of
		 * rx_missed_errors, and basically no packet is delivered to
		 * uplink layer, this is because of the Flow Director is
		 * incorrectly enabled and will interfere with normal rx/tx.
		 * Here we disable Flow Director by default following the
