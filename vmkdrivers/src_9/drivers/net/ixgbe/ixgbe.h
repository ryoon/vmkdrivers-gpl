...
#define ring_is_latency_enabled(ring) \
	test_bit(__IXGBE_RING_LATENCY, &(ring)->state)
#define set_ring_latency_enabled(ring) \
	set_bit(__IXGBE_RING_LATENCY, &(ring)->state)
#define clear_ring_latency_enabled(ring) \
	clear_bit(__IXGBE_RING_LATENCY, &(ring)->state)
...
#ifdef __VMKLNX__
#define IXGBE_FLAG2_LATENCY_ENABLED              (u32)(1 << 12)
#define IXGBE_FLAG2_DYNAMIC_NETQ_ENABLED         (u32)(1 << 13)
#endif
...
extern void ixgbe_del_mac_filter_by_index(struct ixgbe_adapter *adapter,
					  int index);
extern VMK_ReturnStatus ixgbe_passthru_config(struct ixgbe_adapter *adapter,
						u32 vfIdx, int change,
						void *data);
#endif
