...
#ifdef __VMKLNX__
	if (test_and_set_bit(__IXGBE_RING_CLEAN_BUSY, &tx_ring->clean_state))
		return false;
...
#ifdef __VMKLNX__
	clear_bit(__IXGBE_RING_CLEAN_BUSY, &tx_ring->clean_state);
#endif
...
#ifdef NETIF_F_HW_VLAN_TX
			if (adapter->vlgrp && (vlan_tag & VLAN_VID_MASK))
                            ...
                            int cleaned_count)
{
	...
	while (cleaned_count--) {
		...
		if (!bi->page_dma &&
			...
			bi->page_dma = dma_map_page(pci_dev_to_dev(pdev),
						    bi->page,
			                            bi->page_offset,
			                            (PAGE_SIZE / 2),
			                            DMA_FROM_DEVICE);
		...
		if (!bi->skb) {
			struct sk_buff *skb = netdev_alloc_skb_ip_align(netdev, bufsz);
			bi->skb = skb;
 ...
 **/
static inline struct sk_buff *ixgbe_transform_rsc_queue(struct sk_buff *skb, u64 *count)
{
	...
	while (staterr & IXGBE_RXD_STAT_DD) {
		...
		if (staterr & IXGBE_RXD_STAT_EOP) {
			...
			if (rx_ring->rsc_en) {
				...
				if ((rsc_count) &&
				    (adapter->netdev->mtu+ethhdr_sz < skb->len)) {
					struct iphdr *iph;
					struct tcphdr *tcph;
					iph = (struct iphdr *)(skb->data + ethhdr_sz);
					tcph = (char *)iph + (iph->ihl << 2);
					skb_shinfo(skb)->gso_size = skb->len -
					       (ethhdr_sz + ((iph->ihl << 2) + (tcph->doff << 2)));
			...
			if (adapter->flags2 & IXGBE_FLAG2_RSC_ENABLED) {
				if (IXGBE_RSC_CB(skb)->delay_unmap) {
					dma_unmap_single(pci_dev_to_dev(pdev), IXGBE_RSC_CB(skb)->dma,
					                 rx_ring->rx_buf_len,
					                 DMA_FROM_DEVICE);
...
static void ixgbe_check_overtemp_task(struct work_struct *work)
{
	...
	adapter->flags2 |= IXGBE_FLAG2_IN_TEMP_SENSOR_TASK;
                                     ...
                                     int r_idx)
{
	...
	if (a->rx_ring[r_idx]->netdev_type == IXGBE_NETDEV_CNA) {
...
static int ixgbe_map_rings_to_vectors(struct ixgbe_adapter *adapter)
{
	int q_vectors;
	int fcoe_vector;
	int fcoe_vector_count;
	int fcoe_i = adapter->ring_feature[RING_F_FCOE].indices;
	int fcoe_q = adapter->ring_feature[RING_F_FCOE].mask;
	int q_split;
	int v_start = 0;
	...
	if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED) {
		if (q_vectors > MIN_MSIX_COUNT)
			fcoe_vector_count = fcoe_i;
		else
			fcoe_vector_count = MIN_MSIX_Q_FCOE_VECTORS;
		q_split = fcoe_vector_count / 2;
		fcoe_vector = q_vectors - fcoe_vector_count;
		/* map fcoe queues to vectors at end */
		rxr_remaining = adapter->num_rx_queues - fcoe_i;
		txr_remaining = adapter->num_tx_queues - fcoe_i;
		for (i = 0; i < q_split; i++) {
			rqpv = DIV_ROUND_UP(fcoe_i, q_split - i);
			for (j = 0; j < rqpv; j++) {
				map_vector_to_rxq(adapter,
					fcoe_vector, fcoe_q);
				map_vector_to_txq(adapter,
					fcoe_vector+q_split, fcoe_q++);
				fcoe_i--;
			}
			fcoe_vector++;
		}
		q_vectors -= fcoe_vector_count;
	}
	/*
...
#ifdef __VMKLNX__
	/*
	 * VMware wants tx grouped with tx and rx with rx,
	 * rather than the linux RxTx groupings.
	 */
	q_split = q_vectors / 2;
#endif
...
#ifdef __VMKLNX__
           /* 
            * original driver uses 2kb descriptors, we use 4kb.
            * we want to limit the max aggregation size to 
            * 32kb (4kb * 8 descriptors). 
	    *
	    * if we don't, we hit a secret errata where we can aggregate >64kb
            */
#if (MAX_SKB_FRAGS > 8)
...
#ifdef __VMKLNX__
	adapter->rx_ring[index]->rsc_en = true;
#endif /* __VMKLNX__ */
...
void ixgbe_clear_rscctl(struct ixgbe_adapter *adapter, int index)
{
	...
	IXGBE_WRITE_REG(hw, IXGBE_RSCCTL(j), rscctrl);
#ifdef __VMKLNX__
	adapter->rx_ring[index]->rsc_en = FALSE;
#endif /* __VMKLNX__ */
}
...
#ifdef __VMKLNX__
	/* Make L2 buffer size equal to FCoE buffer size.
 	 * Since 3KB slab is available we are not wasting any memory and also
 	 * this avoids security hole introduced by mismatch in buffer size between 
 	 * L2 and FCoE queues.
	 */
...
#ifndef __VMKLNX__
	/* PR 588735: 
 	* Don't start the tx queues until the device is fully up but
 	* rely on the watchdog_timer to detect a link up and start the 
 	* queues there. Otherwise the stack seems to start sending packets
 	* down as soon as the queues are started even though the carrier is off.
 	*/ 	 
	/* enable transmits */
	netif_tx_start_all_queues(netdev);
	if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
		netif_tx_start_all_queues(adapter->cnadev);
#endif
                         ...
                         struct ixgbe_ring *rx_ring)
{
	...
	for (i = 0; i < rx_ring->count; i++) {
		...
		if (!rx_buffer_info->page)
			continue;
#ifndef __VMKLNX__
		dma_unmap_page(pci_dev_to_dev(pdev), rx_buffer_info->page_dma,
			       PAGE_SIZE / 2, DMA_FROM_DEVICE);
#else
		dma_unmap_page(pci_dev_to_dev(pdev), rx_buffer_info->page_dma,
			       PAGE_SIZE, DMA_FROM_DEVICE);
...
void ixgbe_down(struct ixgbe_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		reg_idx = adapter->rx_ring[i]->reg_idx;
		rxctrl = IXGBE_READ_REG(hw, IXGBE_RXDCTL(reg_idx));
		if (rxctrl & IXGBE_RXDCTL_ENABLE) {
			IXGBE_WRITE_REG(hw, IXGBE_RXDCTL(reg_idx),
					rxctrl & ~IXGBE_RXDCTL_ENABLE);
			rxctrl = IXGBE_READ_REG(hw, IXGBE_RXDCTL(reg_idx));
			if (rxctrl & IXGBE_RXDCTL_ENABLE)
				udelay(100);
		}
	...
	if (adapter->flags2 & IXGBE_FLAG2_TEMP_SENSOR_CAPABLE)
		/*
		 * can't call flush scheduled work for the same reason as
		 * mentioned above for the watchdog task.
		 */
		while (adapter->flags2 & IXGBE_FLAG2_IN_TEMP_SENSOR_TASK)
			msleep(1);
...
static inline bool ixgbe_set_vmdq_queues(struct ixgbe_adapter *adapter)
{
...
#ifdef __VMKLNX__
	/* PR 604556. Tests show that optimal perf is achieved if num_queues = MIN(8, num_online_cpus/4). */
	if ((adapter->hw.mac.type == ixgbe_mac_82599EB) && !(adapter->flags2 & IXGBE_FLAG2_VMDQ_DEFAULT_OVERRIDE)) {
	...
	case (IXGBE_FLAG_VMDQ_ENABLED | IXGBE_FLAG_DCB_ENABLED):
		if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
			vmdq_i += adapter->num_rx_cna_queues;
	...
	if ((adapter->netdev->mtu > ETH_DATA_LEN) && (adapter->num_rx_queues > 4))
	{
		...
		if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
			adapter->num_rx_queues += adapter->num_rx_cna_queues;
		...
		if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
			adapter->num_tx_queues += adapter->num_tx_cna_queues;
	}
...
DPRINTK(PROBE, INFO, "rx_ring[0]->reg_idx = %d\n", adapter->rx_ring[0]->reg_idx);
...
static int __devinit ixgbe_sw_init(struct ixgbe_adapter *adapter)
{
...
#ifdef __VMKLNX__
       /* Disable explicitly the packet split and RSC for ESX following erratum 45 */
       adapter->flags &= ~IXGBE_FLAG_RX_PS_ENABLED;
	...
	adapter->ring_feature[RING_F_DCB].indices = 4;
...
static int ixgbe_setup_all_tx_resources(struct ixgbe_adapter *adapter)
{
	...
	int net_tx_queues = adapter->num_tx_queues - adapter->num_tx_cna_queues;
	for (i = 0; i < adapter->num_tx_queues; i++) {
		if (i < net_tx_queues)
			adapter->tx_ring[i]->netdev_type = IXGBE_NETDEV_NET;
		else
			adapter->tx_ring[i]->netdev_type = IXGBE_NETDEV_CNA;
#ifdef HAVE_DEVICE_NUMA_NODE
...
{
	int i, err = 0;
	int net_rx_queues = adapter->num_rx_queues - adapter->num_rx_cna_queues;

	for (i = 0; i < adapter->num_rx_queues; i++) {
		if (i < net_rx_queues)
			adapter->rx_ring[i]->netdev_type = IXGBE_NETDEV_NET;
		else
			adapter->rx_ring[i]->netdev_type = IXGBE_NETDEV_CNA;
#ifdef HAVE_DEVICE_NUMA_NODE
                        ...
                        unsigned int first)
{
	...
	len = min_t(unsigned int, skb_headlen(skb), total);
	while (len) {
		tx_buffer_info = &tx_ring->tx_buffer_info[i];
		...
		tx_buffer_info->dma = dma_map_single(pci_dev_to_dev(pdev),
		                                     skb->data + offset,
		                                     size, DMA_TO_DEVICE);
		if (dma_mapping_error(pci_dev_to_dev(pdev),
		    tx_buffer_info->dma))
	...
	if (adapter->flags & IXGBE_FLAG_FCOE_ENABLED) {
		...
		if ((skb->protocol == htons(ETH_P_FCOE)) ||
			...
			if (netdev->features & NETIF_F_CNA) {
				...
				tx_flags &= ~0x0fff0000;
				tx_flags |= (vlan_tx_tag_get(skb) & 0xfff) <<
					    IXGBE_TX_FLAGS_VLAN_SHIFT;
				tx_flags |= IXGBE_TX_FLAGS_VLAN;
			}
		}
...
xmit_fcoe:
	...
	tx_map_count = ixgbe_tx_map(adapter, tx_ring, skb, tx_flags, first);
	if (tx_map_count < 0) {
...
static int __init ixgbe_init_module(void)
{
	...
	if (!vmk_set_module_version("%s", ixgbe_driver_version)) {
		printk(KERN_INFO "Incompatible Module Version");
		return -ENODEV;
	}
