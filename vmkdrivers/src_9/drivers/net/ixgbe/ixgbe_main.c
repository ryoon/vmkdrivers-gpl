...
#define pci_enable_sriov(dev,vfs) \
        (vmklnx_enable_vfs((dev), (vfs), vmklnx_store_pt_ops, \
			   ixgbe_passthru_ops) != (vfs) ? -ENOTSUPP : 0)
				   ...
				   struct ixgbe_rx_buffer *bi)
{
	...
	if (likely(!skb)) {
		...
		/*
		 * Temporary Workaround for ESX60 issue where cb is corrupted
		 * with FCoE.  Causing PSOD HSD 4707647.  Need to remove after
		 * issue is fully root-caused and fixed.
		 */
		if (IXGBE_CB(skb)->delay_unmap)
			IXGBE_CB(skb)->delay_unmap = 0;
				  ...
				  int budget)
{
	...
	while (ixgbe_test_staterr(rx_desc, IXGBE_RXD_STAT_DD)) {
		...
		len = ixgbe_get_hlen(rx_ring, rx_desc);
#ifdef __VMKLNX__
		/* small packet padding for queue-to-queue loopback */
		if (ixgbe_test_staterr(rx_desc, IXGBE_RXD_STAT_LB)
		    && len < ETH_ZLEN) {
			memset(skb->data+len, 0, (ETH_ZLEN-len));
			len = ETH_ZLEN;
		}
#endif
...
#ifdef __VMKLNX__
                if ((rx_desc->wb.lower.lo_dword.hs_rss.pkt_info &
                     IXGBE_RXDADV_RSSTYPE_MASK) != 0) {
                   u8 q_id = ring_queue_index(rx_ring);
                   if (q_id >= (adapter->num_rx_pools - 1)) {
                      /*
                       * Although hardware gives us the rss hash for all pkts
                       * let us just set it in the skb for pkts that
                       * came through the RSS queues for now.
                       *
                       * This way the higher layers can easily figure out the
                       * pkt was from an RSS queue. We can set the rss hash
                       * for all packets once we identify an extensible way
                       * to let the higher layers know which of the queues
                       * are rss queues and which ones are not.
                       */
                      rss_skb_put_info(skb,
                                       rx_desc->wb.lower.hi_dword.rss,
                                       (vmklnx_rss_type)(rx_desc->wb.lower.lo_dword.hs_rss.pkt_info &
...
free_queue_irqs:
	...
	/*
	 * should not clear this flag as we need it to identify
	 * if we have malloced multiple vectors while freeing q vectors.
	 *
...
static void ixgbe_setup_mrqc(struct ixgbe_adapter *adapter)
{
	...
	case (IXGBE_FLAG_RSS_ENABLED | IXGBE_FLAG_SRIOV_ENABLED):
	case (IXGBE_FLAG_RSS_ENABLED | IXGBE_FLAG_VMDQ_ENABLED 
				     | IXGBE_FLAG_SRIOV_ENABLED):
...
#else
			/* 
			 * In ESX, restricting max aggregated
			 * to 24K gives better perf compared 48K
			 */
			rscctrl |= IXGBE_RSCCTL_MAXDESC_8;
#endif
...
static void ixgbe_configure_virtualization(struct ixgbe_adapter *adapter)
{
	...
	if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
		gcr_ext |= IXGBE_GCR_EXT_VT_MODE_32;
		 ...
		 adapter->ring_feature[RING_F_RSS].indices == 4)
		gcr_ext |= IXGBE_GCR_EXT_VT_MODE_32;
	else
...
#ifdef __VMKLNX__
	/* Only enable anti-spoofing on the VF pools.  The shared code has
	 * turned on anti-spoofing on all the pools except default, but we 
	 * only want it enabled for VF pools */
	if (adapter->num_vfs != 0)
	{
		...
		/* Only enable anti-spoofing on VF pools */
		for (i=0; i < adapter->num_vfs ; i++)
		{
			pfvfspoof = IXGBE_READ_REG(hw, IXGBE_PFVFSPOOF(i >> 3));
			pfvfspoof |= (1 << (i % 8));
			IXGBE_WRITE_REG(hw, IXGBE_PFVFSPOOF(i >> 3), pfvfspoof);
		}
	}
#endif /* __VMKLNX__ */
...
static inline bool ixgbe_set_vmdq_queues(struct ixgbe_adapter *adapter)
{
	...
	if (adapter->flags & IXGBE_FLAG_RSS_ENABLED) {
		...
		adapter->num_tx_queues = min(MAX_TX_QUEUES, vmdq_i + 1); /* +1 for the RSS pool */
				       ...
				       int vectors)
{
	...
	if(!(adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)) {
		/*
		 * MIN_MSIX_COUNT includes 2 vectors
		 * for FCoE, but we won't need them
		 * if CNA is disabled
		 */
 ...
 * will also try to cache the proper offsets if RSS/DCB/CNA/SRIOV are all
		...
		case ixgbe_mac_X540:
			for (i = 0; i < adapter->num_rx_queues; i++) {
				...
				if (i >= adapter->num_rx_queues - f->indices)
					adapter->rx_ring[i]->reg_idx += (adapter->fcoe.tc);
#endif /* IXGBE_FCOE */
			}
			for (i = 0; i < adapter->num_tx_queues; i++) {
...
#ifndef __VMKLNX__
	/*
	* To avoid netq QueuePair confusions
	*/
...
#ifdef __VMKLNX__
			/*
			 * ixgbe_setup_rx_resources sets the ring to not 
			 * allocated for VMware.  Set default queue to 
			 * allocated so filter calls will not fail.
			 * Other queues will be allocated when used for VMQ
			 */
			if (i==0)
				set_ring_allocated(adapter->rx_ring[0]);
...
static int ixgbe_change_mtu(struct net_device *netdev, int new_mtu)
{
	...
	DPRINTK(PROBE, ERR, "changing MTU from %d to %d\n", netdev->mtu, new_mtu);
...
static int ixgbe_open(struct net_device *netdev)
{
	...
	if (err) {
		e_err(probe, "ixgbe_request_irq failed for %s, errno %d\n",
...
err_req_irq:
	...
	set_bit(__IXGBE_DOWN, &adapter->state);
	/*
	 *  disable RX, here we don't disable TX
	 *  since there is no pkt TXed out here,
	 *  this is because the ixgbe_open has not
	 *  yet finished and the ixgbe_down is set.
	 */
	rxctrl = IXGBE_READ_REG(hw, IXGBE_RXCTRL);
	IXGBE_WRITE_REG(hw, IXGBE_RXCTRL, rxctrl & ~IXGBE_RXCTRL_RXEN);
#ifdef IXGBE_FCOE
...
#else
		/*
		 * The original approach unregister_netdev() will release
		 * the dev->sharedData which is used by Uplink module,
		 * then a PSOD will occer, details is described in PR1137688.
		 * Here we close the device using dev_close() and clear
		 * IFF_DEV_IS_OPEN flag to let up layer know the change.
		 */
		rtnl_lock();
		if (adapter->netdev->gflags & IFF_DEV_IS_OPEN) {
			e_info(probe, "closing %s due to unsupported SFP+\n",
				adapter->netdev->name);
			dev_close(adapter->netdev);
			adapter->netdev->gflags &= ~IFF_DEV_IS_OPEN;
		}
		rtnl_unlock();
#endif
	}
...
ixgbe_passthru_ops(struct net_device *netdev, vmk_NetPTOP op, void *pargs)
{
	...
	case VMK_NETPTOP_VF_SET_MAC:
	{
		...
		if (is_zero_ether_addr(args->mac)) {
			...
			DPRINTK(PROBE, ERR, "Setting ndo MAC addr"
					" %02x:%02x:%02x:%02x:%02x:%02x"
					" for vf %u\n",
					args->mac[0], args->mac[1], args->mac[2],
					args->mac[3], args->mac[4], args->mac[5], args->vf);
	...
	case VMK_NETPTOP_VF_GET_QUEUE_STATS:
	{
		...
		ret = ixgbe_vf_get_stats(netdev, args->vf,
						args->numTxQueues,
						args->numRxQueues,
						args->tqStats,
						args->rqStats);
	...
	case VMK_NETPTOP_VF_SET_MTU:
	{
		...
		ret = ixgbe_set_vf_lpe(adapter, args->vf, args->mtu);
		break;
	}
	default:
...
VMK_ReturnStatus
ixgbe_passthru_config(struct ixgbe_adapter *adapter, u32 vfIdx, int change,
			void *data)
{
	vmk_NetVFCfgInfo cfginfo;
	VMK_ReturnStatus ret;
	struct vf_vlan *guest_vlan = 0;
	u32 *new_mtu = 0;
	u8 guest_vid[512];
	s16 array_index;
	u8 bit_index;
	...
	case VMK_CFG_MAC_CHANGED:
		printk("ixgbe_passthru_config: MAC Address changed");
		...
		DPRINTK(PROBE, ERR, "Guest OS requesting MAC addr %02x:%02x:%02x:%02x:%02x:%02x"
				" for VF %d\n", cfginfo.macAddr[0],
				cfginfo.macAddr[1], cfginfo.macAddr[2],cfginfo.macAddr[3],
				cfginfo.macAddr[4], cfginfo.macAddr[5], vfIdx);
		break;
	case VMK_CFG_GUEST_VLAN_ADD:
	case VMK_CFG_GUEST_VLAN_REMOVE:
		guest_vlan = (struct vf_vlan*)data;
		DPRINTK(PROBE, ERR, "Guest OS requesting %s of vlan %d\n",
		guest_vlan->add ? "addition" : "deletion", guest_vlan->vid);
		memset(guest_vid, 0, 512 * sizeof(u8));
		
		array_index = (guest_vlan->vid >> 3) & 0x1FF;
		bit_index = guest_vlan->vid & 0x7;
		bit = (1 << bit_index);
		guest_vid[array_index] |= bit;
		DPRINTK(PROBE, ERR, "vid: %d, Bit index: %d, array index: %d, bit: %x\n", guest_vlan->vid, bit_index, array_index, bit);
		if (guest_vlan->add == TRUE)
			cfginfo.cfgChanged = VMK_CFG_GUEST_VLAN_ADD;
		else
			cfginfo.cfgChanged = VMK_CFG_GUEST_VLAN_REMOVE;

		memcpy(cfginfo.vlan.guestVlans, guest_vid,
			sizeof(cfginfo.vlan.guestVlans));
		DPRINTK(PROBE, ERR, "Passing bit array to kernel\n");
		break;
	case VMK_CFG_MTU_CHANGED:
		new_mtu = (u32*)data;
		cfginfo.cfgChanged = VMK_CFG_MTU_CHANGED;
		DPRINTK(PROBE, ERR, "Guest OS requesting MTU change to %d\n",
			*new_mtu);
		memcpy(&cfginfo.mtu, new_mtu, sizeof(cfginfo.mtu));
		break;
	default:
		printk("Invalid VF configuration change request.\n");
		return VMK_FAILURE;
	}

	return vmklnx_configure_net_vf(adapter->pdev, (void *)&cfginfo, vfIdx);
}
#endif
				 ...
				 const struct pci_device_id *ent)
{
	...
	//Enable LATENCY NETQ
	adapter->flags2 |= IXGBE_FLAG2_LATENCY_ENABLED;
	...
	if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED) {
		err = ixgbe_cna_enable(adapter);
		if (err)
			e_dev_err("ixgbe_cna_enable failed "
				  "return value %d\n", err);
	}
