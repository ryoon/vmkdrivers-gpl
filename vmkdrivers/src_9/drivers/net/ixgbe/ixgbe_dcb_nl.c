...
static u8 ixgbe_get_tc_from_up(struct net_device *netdev, u8 up)
{
	...
	if (adapter->dcb_set_bitmap & BIT_PG_RX) {
		struct ixgbe_dcb_config *dcb_cfg = &adapter->temp_dcb_cfg;
		for (i = 0; i < MAX_TRAFFIC_CLASS; i++) {
			if (dcb_cfg->tc_config[i].path[0].up_to_tc_bitmap & (1 << up))
				return i;
		}
	} else {
		...
		struct ixgbe_dcb_config *dcb_cfg)
{
	...
	pfc_setting = dcb_cfg->tc_config[tc].dcb_pfc;
	adapter->dcb_set_bitmap &= BIT_APP_UPCHG;

        DPRINTK(PROBE, INFO,
		"fcoeup(%d) fcoetc(%d) dcb_set_bitmap(%x) pfc_mode(%d)\n",
		adapter->fcoe.up,
		adapter->fcoe.tc,
	...
	for (j = 0; j < 2; j++) {
		for (i = 0; i < adapter->dcb_cfg.num_tcs.pg_tcs; i++) {
		        DPRINTK(PROBE, INFO,
				"s_cfg   tc(%x) path(%x) map(%x) pfc(%d) "
				" pct(%d) id(%d) id_pct(%d)\n",
				i, j,
				dcb_cfg->tc_config[i].path[j].up_to_tc_bitmap,
				dcb_cfg->tc_config[i].dcb_pfc,
	...
	for (j = 0; j < 2; j++) {
		for (i = 0; i < adapter->dcb_cfg.num_tcs.pg_tcs; i++) {
		        DPRINTK(PROBE, INFO,
				"tc(%x) path(%x) map(%x - %x) tc_pct(%d - %d)"
				" id(%d - %d) bwg_pct(%d - %d)\n",
				i, j,
			...
			if (dcb_cfg->tc_config[i].dcb_pfc !=
			    adapter->dcb_cfg.tc_config[i].dcb_pfc) {
				printk(KERN_INFO "tc(%x) path(%x) pfc(%x - %x)\n",
					dcb_cfg->tc_config[i].dcb_pfc, 
					 adapter->dcb_cfg.tc_config[i].dcb_pfc);
                             ...
                             u8 idtype, u16 id, u8 up)
{
	...
	case DCB_APP_IDTYPE_ETHTYPE:
		...
		if (id == ETH_P_FCOE) {
			...
			rval = ixgbe_fcoe_setapp(adapter, up, tc);
			if (!rval &&
			    (adapter->flags & IXGBE_FLAG_DCB_ENABLED) &&
			...
			/* if no hardware change, return success */
			if (rval == DCB_NO_HW_CHG)
				rval = 0;
		}
#endif
		break;
