...
u8 bnx2x_emac_enable(struct link_params *params,
				   ...
				   struct link_vars *vars)
{
	...
	if (params->feature_config_flags & FEATURE_CONFIG_PFC_ENABLED) {
		DP(NETIF_MSG_LINK, "PFC is enabled\n");
		val |= (1<<6);
	} else if (vars->flow_ctrl & BNX2X_FLOW_CTRL_RX)
		val |= (1<<5);
	wb_data[0] = val;
	wb_data[1] = 0;
				   ...
				   u8 is_lb)
{
	...
	if (params->feature_config_flags & FEATURE_CONFIG_PFC_ENABLED)
		/* When PFC is enabled, set BigMAC to relay control packets
		...
		struct bnx2x_nig_brb_pfc_port_params *pfc_params)
{
	...
	if (set_pfc && pfc_params) {
		 ...
		 **/
		REG_WR(bp, BRB1_REG_PAUSE_1_XOFF_THRESHOLD_0,
			     pause_xoff_th);
		/**
		 * The number of free blocks above which the pause signal to
		 * class 1 of MAC #n is de-asserted. n=0,1
		 **/
		REG_WR(bp, BRB1_REG_PAUSE_1_XON_THRESHOLD_0,
			     pause_xon_th);
		/**
		 * The number of free blocks below which the full signal to
		 * class 1 of MAC #n is asserted. n=0,1
		 **/
		REG_WR(bp, BRB1_REG_FULL_1_XOFF_THRESHOLD_0,
			     full_xoff_th);
		/**
...
u8 bnx2x_pbf_update(struct link_params *params, u32 flow_ctrl,
			   u32 line_speed)
{
	struct bnx2x *bp = params->bp;
	u8 port = params->port;
				   ...
				   struct bnx2x_phy *phy)
{
	...
	if (CHIP_IS_E2(bp)) {
		if (phy->flags & FLAGS_4_PORT_MODE) {
			/**
			 * In E2, 4-port mode, the port within the XGXS is
			 * controlled by setting the port's specific AER.
			 * The AER is set using the 5 MSB which define the
			 * DEVAD_STRAP, and the LSB which stands for the port
			 * id within the XGXS. The ports starts counting from 1
			 */
			aer_val = 0x0800 + (BP_PATH(bp)) +
				params->port*2;
			DP(NETIF_MSG_LINK, "AER path:port [%x:%x] , aer val ="
				       " 0x%x\n", BP_PATH(bp),
				   params->port, aer_val);
		} else {
...
{
	struct bnx2x *bp = params->bp;
	u8 port, reset_xgxs = 1;
	DP(NETIF_MSG_LINK, "bnx2x_xgxs_deassert\n");
	if (params->phy[INT_PHY].flags & FLAGS_4_PORT_MODE) {
		/**
		 * In 4 port mode, all ports reside in XGXS0, so take it out
		 * of reset only for the first port.
		 */
		DP(NETIF_MSG_LINK, "bnx2x_xgxs_deassert 0x%x\n",
			   REG_RD(bp, MISC_REG_RESET_REG_3));
		if ((REG_RD(bp, MISC_REG_RESET_REG_3) &
		     XGXS_RESET_BITS) == XGXS_RESET_BITS)
			reset_xgxs = 0;
		port = 0;
		REG_WR(bp, MISC_REG_XGXS0_CTRL_MD_ST, 0);
	} else
		port = params->port;
	if (reset_xgxs) {
		u32 val;
		...
		REG_WR(bp, GRCBASE_MISC +
			     MISC_REGISTERS_RESET_REG_3_CLEAR, val);
		udelay(500);
		REG_WR(bp, GRCBASE_MISC + MISC_REGISTERS_RESET_REG_3_SET,
			     val);
	}
...
static u8 bnx2x_xgxs_quad_read_status(struct bnx2x_phy *phy,
				      struct link_params *params,
				      struct link_vars *vars)
{
	u16 sgmii_link;
	...
	CL45_RD_OVER_CL22(bp, phy,
				...
				&sgmii_link);
	DP(NETIF_MSG_LINK, "port [%x:%x] sgmii_link = 0x%x\n", BP_PATH(bp),
		   params->port, sgmii_link);
	if ((sgmii_link & (MDIO_SERDES_DIGITAL_A_1000X_STATUS1_SGMII |
		...
		vars->phy_link_up = 0;
	if (sgmii_link & MDIO_SERDES_DIGITAL_A_1000X_STATUS1_DUPLEX)
		vars->duplex = DUPLEX_FULL;
	else
		vars->duplex = DUPLEX_HALF;
	...
	if (vars->phy_link_up) {
		...
		case MDIO_SERDES_DIGITAL_A_1000X_STATUS1_SPEED_10M:
			vars->line_speed = SPEED_10;
			...
			break;
		case MDIO_SERDES_DIGITAL_A_1000X_STATUS1_SPEED_100M:
			vars->line_speed = SPEED_100;
			if (vars->duplex == DUPLEX_FULL)
			...
			break;
		case MDIO_SERDES_DIGITAL_A_1000X_STATUS1_SPEED_1G:
			vars->line_speed = SPEED_1000;
			if (vars->duplex == DUPLEX_FULL)
			...
			break;
		default:
			DP(NETIF_MSG_LINK, "link speed unsupported"
				       " 1000x_link_status 0x%x\n", sgmii_link);
			return -EINVAL;
		}
		DP(NETIF_MSG_LINK, "phy link up @ %d Mbps\n", vars->line_speed);
	} else {
		DP(NETIF_MSG_LINK, "phy link down\n");
				     ...
				     struct link_vars *vars)
{
	...
	DP(NETIF_MSG_LINK, "gp_status 0x%x  phy_link_up %x line_speed %d\n",
...
static u8 bnx2x_init_xgxs_quad(struct bnx2x_phy *phy,
			       struct link_params *params,
			       struct link_vars *vars)
{
	vars->phy_flags = PHY_XGXS_FLAG | PHY_SGMII_FLAG;
	bnx2x_set_aer_mmd_xgxs(params, phy);
	/* reset the SerDes and wait for reset bit return low */
	return bnx2x_reset_unicore(params, phy, 0);
}
...
static void bnx2x_set_xgxs_quad_loopback(struct bnx2x_phy *phy,
					 struct link_params *params)
{
	u16 lane_ctrl, g_port;
	u16 mii_ctrl;
	struct bnx2x *bp = params->bp;
	g_port = (BP_PATH(bp) + (params->port<<1));
	CL45_RD_OVER_CL22(bp, phy,
			    ...
			    u32 led_idx, u32 value)
{
	...
	case 5: /* TRAFFIC led */
		...
		if (reg_val == 1) {
			...
			REG_WR(bp, emac_base + EMAC_REG_EMAC_LED,
				     reg_val);
		 ...
		 struct link_vars *vars, u8 mode, u32 speed)
{
	...
	if (params->feature_config_flags &
	    FEATURE_CONFIG_EMUL_DISABLE_EMAC)
		return rc;
	switch (mode) {
...
u8 bnx2x_link_update(struct link_params *params, struct link_vars *vars)
{
	...
	if (params->phy[INT_PHY].flags & FLAGS_4_PORT_MODE)
		bnx2x_set_aer_mmd_xgxs(params, &params->phy[INT_PHY]);
					      ...
					      u8 port)
{
	...
	/* poll then timeout after 300ms for code download via SPI port - related
	 * to 8073 sometimes taking longer to download */
	do
	{
		count++;
		if(count > 300){
...
static void bnx2x_8073_8727_set_xaui_low_power_mode(struct bnx2x *bp,
						  struct bnx2x_phy *phy)
{
	u16 val;
	bnx2x_cl45_read(bp, phy,
				 ...
				 struct link_vars *vars)
{
	...
	bnx2x_8073_8727_set_xaui_low_power_mode(bp, phy);
...
u8 bnx2x_8705_read_status(struct bnx2x_phy *phy,
			  struct link_params *params,
			  struct link_vars *vars)
{
	u8 link_up = 0;
				      ...
				      u8 tx_en)
{
	...
	DP(NETIF_MSG_LINK, "Setting transmitter tx_en=%x for port %d\n",
		   tx_en, port);
				  ...
				  struct link_params *params)
{
	...
	netdev_err(bp->dev,  "Warning: "
			 "Unqualified SFP+ module "
			 "detected, Port %d from %s part number %s\n"
			, params->port,
				       ...
				       u16 edc_mode)
{
	...
	if (edc_mode == EDC_MODE_LIMITING) {
		...
		/**
		* Changing to LRM mode takes quite few seconds. So do it only if
		* current mode is limiting (default is LRM)
		*/
				  ...
				  struct link_vars *vars)
{
	...
	bnx2x_cl45_read(bp, phy,
			...
			&val2);
	DP(NETIF_MSG_LINK, "BCM848xx: PMD_SIGNAL 1.a811(0) = 0x%x\n", val2);
	...
	case LED_MODE_OPER:
		...
		if((params->hw_led_mode << SHARED_HW_CFG_LED_MODE_SHIFT ) ==
			...
			if(!((val &
				...
				DP(NETIF_MSG_LINK, "Seting MDIO_PMA_REG_8481_LINK_SIGNAL\n");
				bnx2x_cl45_write(bp, phy,
						MDIO_PMA_DEVAD,
						MDIO_PMA_REG_8481_LINK_SIGNAL,
						0xa492);
...
u8 bnx2x_7101_read_status(struct bnx2x_phy *phy,
			  struct link_params *params,
			  struct link_vars *vars)
{
	struct bnx2x *bp = params->bp;
...
static struct bnx2x_phy phy_xgxs_quad = {
	.type		= PORT_HW_CFG_XGXS_EXT_PHY_TYPE_DIRECT,
	.addr		= 0xff,
	.flags		= (FLAGS_4_PORT_MODE |
	...
	/* req_duplex = */0,
	/* rsrv = */0,
	.config_init	= (config_init_t)bnx2x_init_xgxs_quad,
	.read_status   	= (read_status_t)bnx2x_xgxs_quad_read_status,
	.link_reset   	= (link_reset_t)NULL,
	.config_loopback = (config_loopback_t)bnx2x_set_xgxs_quad_loopback,
	.format_fw_ver	= (format_fw_ver_t)NULL,
	.hw_reset	= (hw_reset_t)NULL,
	/*.set_link_led = */(set_link_led_t)NULL,
	/*.phy_specific_func = */(phy_specific_func_t)NULL
				 ...
				 struct bnx2x_phy *phy)
{
	...
	case SWITCH_CFG_10G:
		phy_addr = REG_RD(bp,
					...
					port * 0x18);
		if (CHIP_IS_E2(bp) &&
		    REG_RD(bp, MISC_REG_PORT4MODE_EN))
			*phy = phy_xgxs_quad;
		else
			*phy = phy_xgxs;
		break;
...
u8 bnx2x_phy_init(struct link_params *params, struct link_vars *vars)
{
	...
	if (CHIP_REV_IS_FPGA(bp)) {
		...
		if (params->loopback_mode == LOOPBACK_EMAC)
	...
	if (CHIP_REV_IS_EMUL(bp)) {
		...
		val = REG_RD(bp, (BP_PATH(bp) ?
					MISC_REG_GENERIC_CR_1 :
					MISC_REG_GENERIC_CR_0));
		DP(NETIF_MSG_LINK, "MISC_REG_GENERIC_CR_0 is %x\n", val);

		if (params->feature_config_flags &
		    FEATURE_CONFIG_EMUL_DISABLE_BMAC) {
			vars->line_speed = SPEED_1000;
			vars->link_status = (LINK_STATUS_LINK_UP |
					     LINK_1000XFD);
			if (params->loopback_mode == LOOPBACK_EMAC)
