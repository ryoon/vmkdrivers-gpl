...
static void bnx2x_init_pxp_arb(struct bnx2x *bp, int r_order, int w_order)
{
	...
	/* Todo: E3 will have  a different initialization value
	 * 0x4<< w_order
	 */
	if (CHIP_IS_E2(bp))
...
void bnx2x_ilt_client_init_op_ilt(struct bnx2x *bp, struct bnx2x_ilt *ilt, struct ilt_client_info *ilt_cli, u8 initop)
{
	int i;
...
void bnx2x_ilt_client_init_op(struct bnx2x *bp, struct ilt_client_info *ilt_cli, u8 initop)
{
	struct bnx2x_ilt *ilt = BP_ILT(bp);
...
static void bnx2x_ilt_client_id_init_op(struct bnx2x *bp, int cli_num, u8 initop)
{
	struct bnx2x_ilt *ilt = BP_ILT(bp);
	struct ilt_client_info *ilt_cli = &ilt->clients[cli_num];
