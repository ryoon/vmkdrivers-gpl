...
#if (!defined(__VMKLNX__) || !defined(BNX2X_ESX_DYNAMIC_NETQ)) /* BNX2X_UPSTREAM */
...
int bnx2x_nic_load(struct bnx2x *bp, int load_mode)
{
	...
	for_each_nondefault_eth_queue_(bp, i) {
		if (IS_PF(bp))
	...
	if (rc) {
       ...
       !defined(BNX2X_ESX_DYNAMIC_NETQ)) /* ! BNX2X_UPSTREAM */
...
netdev_tx_t bnx2x_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* drop the packet if the queue is not allocated yet
	 * possible for dynamic netq allocation (no need to do it for CNIC)
	 */
#if defined(BNX2X_ESX_CNA)
	if (txq_index != FCOE_TXQ_IDX(bp) && txq_index > BNX2X_NUM_TX_NETQUEUES(bp)) {
#else
	if (txq_index > BNX2X_NUM_TX_NETQUEUES(bp)) {
#endif
		DP(NETIF_MSG_TX_QUEUED, "drop skb on queue[%d]\n", txq_index);
		dev_kfree_skb_any(skb);
		return NETDEV_TX_OK;
	}

#ifdef BNX2X_ESX_DYNAMIC_NETQ
	struct bnx2x_fastpath *fp = &bp->fp[txq_index];

	if (txq_index <= BNX2X_NUM_TX_NETQUEUES(bp) &&
	    unlikely(!BNX2X_IS_NETQ_TX_QUEUE_ALLOCATED(fp))) {
		DP(NETIF_MSG_TX_QUEUED, "drop skb on queue[%d]\n", txq_index);
		dev_kfree_skb_any(skb);
		return NETDEV_TX_OK;
	}
