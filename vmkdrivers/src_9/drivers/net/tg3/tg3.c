...
#define DRV_MODULE_VERSION	\
	__stringify(TG3_MAJ_NUM) "." __stringify(TG3_MIN_NUM) "b.v50.1"
#define DRV_MODULE_RELDATE	"April 03, 2012"
...
static int tg3_power_down_prepare(struct tg3 *tp)
{
	...
	if (device_should_wake) {
		...
		if (!(tp->phy_flags & TG3_PHYFLG_PHY_SERDES)) {
			...
			if (tp->phy_flags & TG3_PHYFLG_MII_SERDES)
				mac_mode = MAC_MODE_PORT_MODE_GMII;
			else if (tp->phy_flags & TG3_PHYFLG_KEEP_LINK_ON_PWRDN) {
				if (tp->link_config.active_speed == SPEED_1000)
					mac_mode = MAC_MODE_PORT_MODE_GMII;
				else
					mac_mode = MAC_MODE_PORT_MODE_MII;
...
static int tg3_phy_pull_flowctrl(struct tg3 *tp, u32 lcl)
{
	int err;
	u32 val, res;

	err = tg3_readphy(tp, MII_LPA, &val);
	if (err)
		return err;

	if (tp->phy_flags & TG3_PHYFLG_ANY_SERDES)
		res = tg3_resolve_flowctrl_1000X(lcl, val);
	else
		res = mii_resolve_flowctrl_fdx(lcl, val);

	val = 0;

	if (tr32(MAC_RX_MODE) & RX_MODE_FLOW_CTRL_ENABLE)
		val |= FLOW_CTRL_RX;

	if (tr32(MAC_TX_MODE) & TX_MODE_FLOW_CTRL_ENABLE)
		val |= FLOW_CTRL_TX;

	if (res == val) {
		/* We can't be sure if flow control was really autonegotiated.
		 * We'll assume that is the case, because that's what we want anyways.
		 */
		tg3_flag_set(tp, PAUSE_AUTONEG);
	} else {
		/* If the flow control settings in the MAC differ from what the
		 * autoneg advertisements would have resolved to, then the flow
		 * control must have been forced.
		 */
		tp->link_config.flowctrl = val;
...
static int tg3_phy_pull_config(struct tg3 *tp)
{
	...
	if (!(val & BMCR_ANENABLE)) {
		...
		val = tr32(MAC_RX_MODE);
		if (val & RX_MODE_FLOW_CTRL_ENABLE)
			tp->link_config.flowctrl |= FLOW_CTRL_RX;

		val = tr32(MAC_TX_MODE);
		if (val & TX_MODE_FLOW_CTRL_ENABLE)
			tp->link_config.flowctrl |= FLOW_CTRL_TX;

		/* Assume autoneg flow control is desirable */
		tg3_flag_set(tp, PAUSE_AUTONEG);
	...
	if (!(tp->phy_flags & TG3_PHYFLG_ANY_SERDES)) {
		...
		adv = tg3_decode_flowctrl_1000T(val);
	...
	err = tg3_phy_pull_flowctrl(tp, lclfc);

done:
	return err;
}

static int tg3_phy_pull_status(struct tg3 *tp)
{
	u32 val;
	int i, err;
	...
	for (i = 0; i < 2000; i++) {
		err = tg3_readphy(tp, MII_TG3_AUX_STAT, &val);
		if (err)
			return err;
	...
	tg3_aux_stat_to_speed_duplex(tp, val,
				     &tp->link_config.active_speed,
				     &tp->link_config.active_duplex);

	tp->link_config.active_flowctrl = tp->link_config.flowctrl;
...
{
	u64 ns = timecounter_cyc2time(&tp->clock, hwclock);
...
static int tg3_rx(struct tg3_napi *tnapi, int budget)
{
	...
	while (sw_idx != hw_idx && budget > 0) {
...
#endif
		drop_it:
			tg3_recycle_rx(tnapi, tnapi->srcprodring, tpr,
				       opaque_key, desc_idx, *post_ptr);
		drop_it_no_recycle:
		...
		if ((desc->type_flags & RXD_FLAG_PTPSTAT_MASK) == RXD_FLAG_PTPSTAT_PTPV1 ||
			...
			u64 hwclock = tr32(TG3_RX_TSTAMP_LSB);
			hwclock |= (u64)tr32(TG3_RX_TSTAMP_MSB) << 32;
			tg3_hwclock_to_timestamp(tp, hwclock,
						 skb_hwtstamps(skb));
		}
#endif /* BCM_HAS_IEEE1588_SUPPORT */
...
next_pkt_nopost:
	...
	if (!tg3_flag(tp, ENABLE_RSS)) {
		...
		if (tnapi != &tp->napi[1]) {
			tp->rx_refill = true;
			tg3_netif_rx_schedule(tp->dev, &tp->napi[1].napi);
		}
	}
			    ...
			    u32 mss, u32 vlan)
{
	...
	if (tg3_flag(tp, NO_TSO_BD_LIMIT) && mss)
		dma_limit = 0;
...
static int tg3_reset_hw(struct tg3 *tp, int reset_phy)
{
	...
	if (tp->rxptpctl || tg3_flag(tp, TX_TSTAMP_EN)) {
		tw32(TG3_RX_PTP_CTL,
...
static inline u32 tg3_irq_count(struct tg3 *tp)
{
	...
	irqcnt = max(tp->rxq_cnt, tp->txq_cnt);
	if (irqcnt > 1) {
		/* We want as many rx rings enabled as there are cpus.
		 * In multiqueue MSI-X mode, the first MSI-X vector only deals
		 * with link interrupts, etc, so we add one to the number of
		 * vectors we are requesting.
		 */
		irqcnt = min_t(unsigned, irqcnt + 1, tp->irq_max);
	}

#ifdef TG3_VMWARE_NETQ_ENABLE
	if (tg3_flag(tp, IOV_CAPABLE))
		irqcnt = tg3_netq_tune_vector_count(tp);
#endif
...
static bool tg3_enable_msix(struct tg3 *tp)
{
	...
	if (!tp->txq_cnt)
		tp->txq_cnt = min(cpus, tp->txq_max);
	if (!tp->rxq_cnt)
	...
	if (tp->txq_cnt > 1) {
		...
		tp->txq_cnt = min(tp->irq_cnt - 1, tp->txq_cnt);
	}
...
static int tg3_open(struct net_device *dev)
{
	...
	err = tg3_init_hw(tp,
			  (tp->phy_flags & TG3_PHYFLG_KEEP_LINK_ON_PWRDN) == 0);
	if (err) {
		tg3_halt(tp, RESET_KIND_SHUTDOWN, 1);
		tg3_free_rings(tp);
	} else {
...
static int tg3_start(struct tg3 *tp, bool reset_phy)
{
	int i, err;
	...
	/* Make sure the RSS indirection table
	 * does not use unallocated interrupts.
	 */
	tg3_rss_check_indir_tbl(tp);
	...
	netif_tx_start_all_queues(tp->dev);

#ifdef BCM_HAS_FIX_FEATURES
	/* Reset loopback feature if it was turned on while the device
	 * was down make sure that it's installed properly now.
	 */
	if (dev->features & NETIF_F_LOOPBACK)
		tg3_set_loopback(tp->dev, tp->dev->features);
#endif
...
static int tg3_stop(struct tg3 *tp)
{
	int i;
...
#ifdef __VMKLNX__
	/* VMWare has a bug where transmits might be reattempted while
	 * the device is shutting down.  The only way to fix this is to
	 * take the tx_lock, which happens through netif_tx_disable().
	 */
	tg3_netif_stop(tp);
#else
	tg3_napi_disable(tp);
			     ...
			     struct ethtool_channels *channel)
{
	...
	if (netif_running(dev)) {
		...
		if (tp->rxq_req)
			...
			channel->rx_count = min(num_online_cpus(), tp->rxq_max);

		if (tp->txq_req)
			channel->tx_count = tp->txq_req;
		else
			channel->tx_count = min(num_online_cpus(), tp->txq_max);
	}
}
			      ...
			      struct ifreq *ifr, int cmd)
{
	...
	if (netif_running(dev)) {
		grc_mode = tr32(GRC_MODE) & ~GRC_MODE_TIME_SYNC_ENABLE;
		tw32(GRC_MODE, grc_mode);
		...
		if (tp->rxptpctl || tg3_flag(tp, TX_TSTAMP_EN)) {
			...
			grc_mode |= GRC_MODE_TIME_SYNC_ENABLE;
			tw32(GRC_MODE, grc_mode);
		}
	}
...
static int __devinit tg3_get_invariants(struct tg3 *tp)
{
	...
	if (tg3_flag(tp, 5750_PLUS)) {
		...
		if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_5718 ||
			...
			tp->irq_max = TG3_IRQ_MAX_VECS_IOV;
...
#ifdef BCM_HAS_IEEE1588_SUPPORT
static cycle_t tg3_read_clock(const struct cyclecounter *tc)
{
...
static void __devinit tg3_init_hwtimer(struct tg3 *tp)
{
	if (GET_ASIC_REV(tp->pci_chip_rev_id) != ASIC_REV_5719 &&
	    GET_ASIC_REV(tp->pci_chip_rev_id) != ASIC_REV_5720)
		return;
	...
	tp->cycles.read = tg3_read_clock;
	tp->cycles.mask = CLOCKSOURCE_MASK(64);
	tp->cycles.mult = 1;

	/* Set registers so that rollover occurs soon to test this. */
	tw32(TG3_EAV_REF_CLCK_CTL, TG3_EAV_REF_CLCK_CTL_STOP);
	tw32(TG3_EAV_REF_CLCK_LSB, 0x00000000);
	tw32(TG3_EAV_REF_CLCK_MSB, 0xFF800000);
	tw32_f(TG3_EAV_REF_CLCK_CTL, 0x0);
...
#endif
#if defined(BCM_KERNEL_SUPPORTS_8021Q) && defined(BCM_USE_OLD_VLAN_INTERFACE)
...
#ifdef TG3_VMWARE_NETQ_ENABLE
	if (tg3_flag(tp, IOV_CAPABLE) &&
	    tg3_netq_force[tp->vmware.netq.index])
		tg3_netq_init(tp);
#endif
